let dictionary = [
    {en:"a", es:"un"},
    {en:"abandon", es:"abandonar"},
    {en:"ability", es:"capacidad"},
    {en:"able", es:"poder"},
    {en:"abortion", es:"aborto"},
    {en:"about", es:"acerca de"},
    {en:"above", es:"sobre"},
    {en:"abroad", es:"en el extranjero"},
    {en:"absence", es:"ausencia"},
    {en:"absolute", es:"absoluto"},
    {en:"absolutely", es:"absolutamente"},
    {en:"absorb", es:"absorber"},
    {en:"abuse", es:"abuso"},
    {en:"academic", es:"académico"},
    {en:"accept", es:"aceptar"},
    {en:"access", es:"acceso"},
    {en:"accident", es:"accidente"},
    {en:"accompany", es:"acompañar"},
    {en:"accomplish", es:"lograr"},
    {en:"according", es:"según"},
    {en:"account", es:"cuenta"},
    {en:"accurate", es:"preciso"},
    {en:"accuse", es:"acusar"},
    {en:"achieve", es:"lograr"},
    {en:"achievement", es:"logro"},
    {en:"acid", es:"ácido"},
    {en:"acknowledge", es:"reconocer"},
    {en:"acquire", es:"adquirir"},
    {en:"across", es:"a través de"},
    {en:"act", es:"actuar"},
    {en:"action", es:"acción"},
    {en:"active", es:"activo"},
    {en:"activist", es:"activista"},
    {en:"activity", es:"actividad"},
    {en:"actor", es:"actor"},
    {en:"actress", es:"Actriz"},
    {en:"actual", es:"actual"},
    {en:"actually", es:"actualmente"},
    {en:"ad", es:"anuncio"},
    {en:"adapt", es:"adaptar"},
    {en:"add", es:"agregar"},
    {en:"addition", es:"adición"},
    {en:"additional", es:"adicional"},
    {en:"address", es:"dirección"},
    {en:"adequate", es:"adecuado"},
    {en:"adjust", es:"ajustar"},
    {en:"adjustment", es:"ajustamiento"},
    {en:"administration", es:"administración"},
    {en:"administrator", es:"administrador"},
    {en:"admire", es:"admirar"},
    {en:"admission", es:"admisión"},
    {en:"admit", es:"admitir"},
    {en:"adolescent", es:"adolescente"},
    {en:"adopt", es:"adoptar"},
    {en:"adult", es:"adulto"},
    {en:"advance", es:"avanzar"},
    {en:"advanced", es:"avanzado"},
    {en:"advantage", es:"ventaja"},
    {en:"adventure", es:"aventura"},
    {en:"advertising", es:"publicidad"},
    {en:"advice", es:"Consejo"},
    {en:"advise", es:"aconsejar"},
    {en:"adviser", es:"asesor"},
    {en:"advocate", es:"defensor"},
    {en:"affair", es:"amorío"},
    {en:"affect", es:"afectar"},
    {en:"afford", es:"poder pagar"},
    {en:"afraid", es:"temeroso"},
    {en:"African", es:"africano"},
    {en:"African-American", es:"Afroamericano"},
    {en:"after", es:"después"},
    {en:"afternoon", es:"tarde"},
    {en:"again", es:"de nuevo"},
    {en:"against", es:"contra"},
    {en:"age", es:"envejecer"},
    {en:"agency", es:"agencia"},
    {en:"agenda", es:"agenda"},
    {en:"agent", es:"agente"},
    {en:"aggressive", es:"agresivo"},
    {en:"ago", es:"atrás"},
    {en:"agree", es:"estar de acuerdo"},
    {en:"agreement", es:"convenio"},
    {en:"agricultural", es:"agrícola"},
    {en:"ah", es:"ah"},
    {en:"ahead", es:"adelante"},
    {en:"aid", es:"ayuda"},
    {en:"aide", es:"ayudante"},
    {en:"AIDS", es:"SIDA"},
    {en:"aim", es:"apuntar"},
    {en:"air", es:"aire"},
    {en:"aircraft", es:"aeronave"},
    {en:"airline", es:"aerolínea"},
    {en:"airport", es:"aeropuerto"},
    {en:"album", es:"álbum"},
    {en:"alcohol", es:"alcohol"},
    {en:"alive", es:"viva"},
    {en:"all", es:"todo"},
    {en:"alliance", es:"Alianza"},
    {en:"allow", es:"permitir"},
    {en:"ally", es:"aliado"},
    {en:"almost", es:"casi"},
    {en:"alone", es:"solo"},
    {en:"along", es:"a lo largo de"},
    {en:"already", es:"ya"},
    {en:"also", es:"además"},
    {en:"alter", es:"alterar"},
    {en:"alternative", es:"alternativa"},
    {en:"although", es:"a pesar de que"},
    {en:"always", es:"siempre"},
    {en:"AM", es:"SOY"},
    {en:"amazing", es:"increíble"},
    {en:"American", es:"americano"},
    {en:"among", es:"entre"},
    {en:"amount", es:"Monto"},
    {en:"analysis", es:"análisis"},
    {en:"analyst", es:"analista"},
    {en:"analyze", es:"analizar"},
    {en:"ancient", es:"antiguo"},
    {en:"and", es:"y"},
    {en:"anger", es:"enfado"},
    {en:"angle", es:"ángulo"},
    {en:"angry", es:"enojado"},
    {en:"animal", es:"animal"},
    {en:"anniversary", es:"aniversario"},
    {en:"announce", es:"anunciar"},
    {en:"annual", es:"anual"},
    {en:"another", es:"otro"},
    {en:"answer", es:"respuesta"},
    {en:"anticipate", es:"prever"},
    {en:"anxiety", es:"ansiedad"},
    {en:"any", es:"ningún"},
    {en:"anybody", es:"cualquiera"},
    {en:"anymore", es:"nunca más"},
    {en:"anyone", es:"alguien"},
    {en:"anything", es:"cualquier cosa"},
    {en:"anyway", es:"de todos modos"},
    {en:"anywhere", es:"en cualquier lugar"},
    {en:"apart", es:"aparte"},
    {en:"apartment", es:"Departamento"},
    {en:"apparent", es:"aparente"},
    {en:"apparently", es:"aparentemente"},
    {en:"appeal", es:"apelación"},
    {en:"appear", es:"aparecer"},
    {en:"appearance", es:"apariencia"},
    {en:"apple", es:"manzana"},
    {en:"application", es:"solicitud"},
    {en:"apply", es:"solicitar"},
    {en:"appoint", es:"nombrar"},
    {en:"appointment", es:"cita"},
    {en:"appreciate", es:"agradecer"},
    {en:"approach", es:"Acercarse"},
    {en:"appropriate", es:"apropiado"},
    {en:"approval", es:"aprobación"},
    {en:"approve", es:"aprobar"},
    {en:"approximately", es:"aproximadamente"},
    {en:"Arab", es:"árabe"},
    {en:"architect", es:"arquitecto"},
    {en:"area", es:"área"},
    {en:"argue", es:"discutir"},
    {en:"argument", es:"argumento"},
    {en:"arise", es:"aumentar"},
    {en:"arm", es:"brazo"},
    {en:"armed", es:"armado"},
    {en:"army", es:"Ejército"},
    {en:"around", es:"alrededor"},
    {en:"arrange", es:"arreglar"},
    {en:"arrangement", es:"disposición"},
    {en:"arrest", es:"arrestar"},
    {en:"arrival", es:"llegada"},
    {en:"arrive", es:"llegar"},
    {en:"art", es:"Arte"},
    {en:"article", es:"artículo"},
    {en:"artist", es:"artista"},
    {en:"artistic", es:"artístico"},
    {en:"as", es:"como"},
    {en:"Asian", es:"asiático"},
    {en:"aside", es:"aparte"},
    {en:"ask", es:"preguntar"},
    {en:"asleep", es:"dormido"},
    {en:"aspect", es:"aspecto"},
    {en:"assault", es:"asalto"},
    {en:"assert", es:"afirmar"},
    {en:"assess", es:"evaluar"},
    {en:"assessment", es:"evaluación"},
    {en:"asset", es:"activo"},
    {en:"assign", es:"asignar"},
    {en:"assignment", es:"asignación"},
    {en:"assist", es:"ayudar"},
    {en:"assistance", es:"asistencia"},
    {en:"assistant", es:"asistente"},
    {en:"associate", es:"asociar"},
    {en:"association", es:"asociación"},
    {en:"assume", es:"asumir"},
    {en:"assumption", es:"suposición"},
    {en:"assure", es:"asegurar"},
    {en:"at", es:"en"},
    {en:"athlete", es:"atleta"},
    {en:"athletic", es:"atlético"},
    {en:"atmosphere", es:"atmósfera"},
    {en:"attach", es:"adjuntar"},
    {en:"attack", es:"ataque"},
    {en:"attempt", es:"intento"},
    {en:"attend", es:"atender"},
    {en:"attention", es:"atención"},
    {en:"attitude", es:"actitud"},
    {en:"attorney", es:"abogado"},
    {en:"attract", es:"atraer"},
    {en:"attractive", es:"atractivo"},
    {en:"attribute", es:"atributo"},
    {en:"audience", es:"audiencia"},
    {en:"author", es:"autor"},
    {en:"authority", es:"autoridad"},
    {en:"auto", es:"auto"},
    {en:"available", es:"disponible"},
    {en:"average", es:"promedio"},
    {en:"avoid", es:"evitar"},
    {en:"award", es:"otorgar"},
    {en:"aware", es:"consciente"},
    {en:"awareness", es:"conciencia"},
    {en:"away", es:"lejos"},
    {en:"awful", es:"horrible"},
    {en:"baby", es:"bebé"},
    {en:"back", es:"espalda"},
    {en:"background", es:"antecedentes"},
    {en:"bad", es:"malo"},
    {en:"badly", es:"gravemente"},
    {en:"bag", es:"bolso"},
    {en:"bake", es:"hornear"},
    {en:"balance", es:"equilibrio"},
    {en:"ball", es:"bola"},
    {en:"ban", es:"prohibición"},
    {en:"band", es:"banda"},
    {en:"bank", es:"Banco"},
    {en:"bar", es:"bar"},
    {en:"barely", es:"apenas"},
    {en:"barrel", es:"barril"},
    {en:"barrier", es:"barrera"},
    {en:"base", es:"base"},
    {en:"baseball", es:"béisbol"},
    {en:"basic", es:"básico"},
    {en:"basically", es:"básicamente"},
    {en:"basis", es:"base"},
    {en:"basket", es:"cesta"},
    {en:"basketball", es:"baloncesto"},
    {en:"bathroom", es:"cuarto de baño"},
    {en:"battery", es:"batería"},
    {en:"battle", es:"batalla"},
    {en:"be", es:"ser"},
    {en:"beach", es:"playa"},
    {en:"bean", es:"frijol"},
    {en:"bear", es:"soportar"},
    {en:"beat", es:"derrotar"},
    {en:"beautiful", es:"hermosa"},
    {en:"beauty", es:"belleza"},
    {en:"because", es:"porque"},
    {en:"become", es:"convertirse en"},
    {en:"bed", es:"cama"},
    {en:"bedroom", es:"dormitorio"},
    {en:"beer", es:"cerveza"},
    {en:"before", es:"antes de"},
    {en:"begin", es:"comenzar"},
    {en:"beginning", es:"comienzo"},
    {en:"behavior", es:"conducta"},
    {en:"behind", es:"detrás"},
    {en:"being", es:"ser"},
    {en:"belief", es:"creencia"},
    {en:"believe", es:"creer"},
    {en:"bell", es:"campana"},
    {en:"belong", es:"pertenecer"},
    {en:"below", es:"debajo"},
    {en:"belt", es:"cinturón"},
    {en:"bench", es:"mesa de trabajo"},
    {en:"bend", es:"doblar"},
    {en:"beneath", es:"bajo"},
    {en:"benefit", es:"beneficio"},
    {en:"beside", es:"junto a"},
    {en:"besides", es:"además"},
    {en:"best", es:"mejor"},
    {en:"bet", es:"apuesta"},
    {en:"better", es:"mejor"},
    {en:"between", es:"Entre"},
    {en:"beyond", es:"más allá de"},
    {en:"Bible", es:"Biblia"},
    {en:"big", es:"grande"},
    {en:"bike", es:"bicicleta"},
    {en:"bill", es:"factura"},
    {en:"billion", es:"mil millones"},
    {en:"bind", es:"unir"},
    {en:"biological", es:"biológico"},
    {en:"bird", es:"pájaro"},
    {en:"birth", es:"nacimiento"},
    {en:"birthday", es:"cumpleaños"},
    {en:"bit", es:"un poco"},
    {en:"bite", es:"morder"},
    {en:"black", es:"negro"},
    {en:"blade", es:"espada"},
    {en:"blame", es:"culpar"},
    {en:"blanket", es:"cobija"},
    {en:"blind", es:"ciego"},
    {en:"block", es:"cuadra"},
    {en:"blood", es:"sangre"},
    {en:"blow", es:"soplo"},
    {en:"blue", es:"azul"},
    {en:"board", es:"Junta"},
    {en:"boat", es:"bote"},
    {en:"body", es:"cuerpo"},
    {en:"bomb", es:"bomba"},
    {en:"bombing", es:"bombardeo"},
    {en:"bond", es:"vínculo"},
    {en:"bone", es:"hueso"},
    {en:"book", es:"libro"},
    {en:"boom", es:"auge"},
    {en:"boot", es:"bota"},
    {en:"border", es:"frontera"},
    {en:"born", es:"Nació"},
    {en:"borrow", es:"pedir prestado"},
    {en:"boss", es:"jefe"},
    {en:"both", es:"ambos"},
    {en:"bother", es:"molestar"},
    {en:"bottle", es:"botella"},
    {en:"bottom", es:"fondo"},
    {en:"boundary", es:"Perímetro"},
    {en:"bowl", es:"cuenco"},
    {en:"box", es:"caja"},
    {en:"boy", es:"chico"},
    {en:"boyfriend", es:"novio"},
    {en:"brain", es:"cerebro"},
    {en:"branch", es:"rama"},
    {en:"brand", es:"marca"},
    {en:"bread", es:"pan de molde"},
    {en:"break", es:"descanso"},
    {en:"breakfast", es:"desayuno"},
    {en:"breast", es:"pecho"},
    {en:"breath", es:"aliento"},
    {en:"breathe", es:"respirar"},
    {en:"brick", es:"ladrillo"},
    {en:"bridge", es:"puente"},
    {en:"brief", es:"breve"},
    {en:"briefly", es:"brevemente"},
    {en:"bright", es:"brillante"},
    {en:"brilliant", es:"brillante"},
    {en:"bring", es:"traer"},
    {en:"British", es:"británico"},
    {en:"broad", es:"amplio"},
    {en:"broken", es:"roto"},
    {en:"brother", es:"hermano"},
    {en:"brown", es:"marrón"},
    {en:"brush", es:"cepillar"},
    {en:"buck", es:"dólar"},
    {en:"budget", es:"presupuesto"},
    {en:"build", es:"construir"},
    {en:"building", es:"edificio"},
    {en:"bullet", es:"bala"},
    {en:"bunch", es:"racimo"},
    {en:"burden", es:"carga"},
    {en:"burn", es:"Quemadura"},
    {en:"bury", es:"enterrar"},
    {en:"bus", es:"autobús"},
    {en:"business", es:"negocio"},
    {en:"busy", es:"ocupado"},
    {en:"but", es:"pero"},
    {en:"butter", es:"manteca"},
    {en:"button", es:"botón"},
    {en:"buy", es:"comprar"},
    {en:"buyer", es:"comprador"},
    {en:"by", es:"por"},
    {en:"cabin", es:"cabina"},
    {en:"cabinet", es:"gabinete"},
    {en:"cable", es:"cable"},
    {en:"cake", es:"pastel"},
    {en:"calculate", es:"calcular"},
    {en:"call", es:"llamar"},
    {en:"camera", es:"cámara"},
    {en:"camp", es:"acampar"},
    {en:"campaign", es:"Campaña"},
    {en:"campus", es:"instalaciones"},
    {en:"can", es:"poder"},
    {en:"Canadian", es:"canadiense"},
    {en:"cancer", es:"cáncer"},
    {en:"candidate", es:"candidato"},
    {en:"cap", es:"gorra"},
    {en:"capability", es:"capacidad"},
    {en:"capable", es:"capaz"},
    {en:"capacity", es:"capacidad"},
    {en:"capital", es:"capital"},
    {en:"captain", es:"capitán"},
    {en:"capture", es:"capturar"},
    {en:"car", es:"carro"},
    {en:"carbon", es:"carbón"},
    {en:"card", es:"tarjeta"},
    {en:"care", es:"cuidado"},
    {en:"career", es:"carrera profesional"},
    {en:"careful", es:"Cuidado"},
    {en:"carefully", es:"con cuidado"},
    {en:"carrier", es:"transportador"},
    {en:"carry", es:"llevar"},
    {en:"case", es:"caso"},
    {en:"cash", es:"dinero en efectivo"},
    {en:"cast", es:"Elenco"},
    {en:"cat", es:"gato"},
    {en:"catch", es:"captura"},
    {en:"category", es:"categoría"},
    {en:"Catholic", es:"católico"},
    {en:"cause", es:"porque"},
    {en:"ceiling", es:"techo"},
    {en:"celebrate", es:"celebrar"},
    {en:"celebration", es:"celebracion"},
    {en:"celebrity", es:"celebridad"},
    {en:"cell", es:"celda"},
    {en:"center", es:"centrar"},
    {en:"central", es:"central"},
    {en:"century", es:"siglo"},
    {en:"CEO", es:"CEO"},
    {en:"ceremony", es:"ceremonia"},
    {en:"certain", es:"cierto"},
    {en:"certainly", es:"ciertamente"},
    {en:"chain", es:"cadena"},
    {en:"chair", es:"silla"},
    {en:"chairman", es:"presidente"},
    {en:"challenge", es:"desafío"},
    {en:"chamber", es:"cámara"},
    {en:"champion", es:"campeón"},
    {en:"championship", es:"campeonato"},
    {en:"chance", es:"oportunidad"},
    {en:"change", es:"cambio"},
    {en:"changing", es:"cambiando"},
    {en:"channel", es:"canal"},
    {en:"chapter", es:"capítulo"},
    {en:"character", es:"personaje"},
    {en:"characteristic", es:"característica"},
    {en:"characterize", es:"caracterizar"},
    {en:"charge", es:"cargo"},
    {en:"charity", es:"caridad"},
    {en:"chart", es:"gráfico"},
    {en:"chase", es:"perseguir"},
    {en:"cheap", es:"barato"},
    {en:"check", es:"cheque"},
    {en:"cheek", es:"mejilla"},
    {en:"cheese", es:"queso"},
    {en:"chef", es:"cocinero"},
    {en:"chemical", es:"químico"},
    {en:"chest", es:"cofre"},
    {en:"chicken", es:"pollo"},
    {en:"chief", es:"jefe"},
    {en:"child", es:"niño"},
    {en:"childhood", es:"infancia"},
    {en:"Chinese", es:"chino"},
    {en:"chip", es:"chip"},
    {en:"chocolate", es:"chocolate"},
    {en:"choice", es:"elección"},
    {en:"cholesterol", es:"colesterol"},
    {en:"choose", es:"escoger"},
    {en:"Christian", es:"cristiano"},
    {en:"Christmas", es:"Navidad"},
    {en:"church", es:"Iglesia"},
    {en:"cigarette", es:"cigarrillo"},
    {en:"circle", es:"circulo"},
    {en:"circumstance", es:"circunstancia"},
    {en:"cite", es:"citar"},
    {en:"citizen", es:"ciudadano"},
    {en:"city", es:"ciudad"},
    {en:"civil", es:"civil"},
    {en:"civilian", es:"civil"},
    {en:"claim", es:"afirmar"},
    {en:"class", es:"clase"},
    {en:"classic", es:"clásico"},
    {en:"classroom", es:"salón de clases"},
    {en:"clean", es:"limpio"},
    {en:"clear", es:"claro"},
    {en:"clearly", es:"claramente"},
    {en:"client", es:"cliente"},
    {en:"climate", es:"clima"},
    {en:"climb", es:"subida"},
    {en:"clinic", es:"clínica"},
    {en:"clinical", es:"clínico"},
    {en:"clock", es:"reloj"},
    {en:"close", es:"cerca"},
    {en:"closely", es:"cercanamente"},
    {en:"closer", es:"más cerca"},
    {en:"clothes", es:"ropa"},
    {en:"clothing", es:"ropa"},
    {en:"cloud", es:"nube"},
    {en:"club", es:"club"},
    {en:"clue", es:"pista"},
    {en:"cluster", es:"grupo"},
    {en:"coach", es:"entrenador"},
    {en:"coal", es:"carbón"},
    {en:"coalition", es:"coalición"},
    {en:"coast", es:"costa"},
    {en:"coat", es:"Saco"},
    {en:"code", es:"código"},
    {en:"coffee", es:"café"},
    {en:"cognitive", es:"cognitivo"},
    {en:"cold", es:"frío"},
    {en:"collapse", es:"colapso"},
    {en:"colleague", es:"colega"},
    {en:"collect", es:"recoger"},
    {en:"collection", es:"colección"},
    {en:"collective", es:"colectivo"},
    {en:"college", es:"Universidad"},
    {en:"colonial", es:"colonial"},
    {en:"color", es:"color"},
    {en:"column", es:"columna"},
    {en:"combination", es:"combinación"},
    {en:"combine", es:"combinar"},
    {en:"come", es:"venir"},
    {en:"comedy", es:"comedia"},
    {en:"comfort", es:"comodidad"},
    {en:"comfortable", es:"cómodo"},
    {en:"command", es:"mando"},
    {en:"commander", es:"comandante"},
    {en:"comment", es:"comentario"},
    {en:"commercial", es:"comercial"},
    {en:"commission", es:"comisión"},
    {en:"commit", es:"cometer"},
    {en:"commitment", es:"compromiso"},
    {en:"committee", es:"comité"},
    {en:"common", es:"común"},
    {en:"communicate", es:"comunicar"},
    {en:"communication", es:"comunicación"},
    {en:"community", es:"comunidad"},
    {en:"company", es:"empresa"},
    {en:"compare", es:"comparar"},
    {en:"comparison", es:"comparación"},
    {en:"compete", es:"competir"},
    {en:"competition", es:"competencia"},
    {en:"competitive", es:"competitivo"},
    {en:"competitor", es:"competidor"},
    {en:"complain", es:"quejarse"},
    {en:"complaint", es:"queja"},
    {en:"complete", es:"completo"},
    {en:"completely", es:"completamente"},
    {en:"complex", es:"complejo"},
    {en:"complicated", es:"Complicado"},
    {en:"component", es:"componente"},
    {en:"compose", es:"componer"},
    {en:"composition", es:"composición"},
    {en:"comprehensive", es:"exhaustivo"},
    {en:"computer", es:"computadora"},
    {en:"concentrate", es:"concentrado"},
    {en:"concentration", es:"concentración"},
    {en:"concept", es:"concepto"},
    {en:"concern", es:"preocupación"},
    {en:"concerned", es:"preocupado"},
    {en:"concert", es:"concierto"},
    {en:"conclude", es:"concluir"},
    {en:"conclusion", es:"conclusión"},
    {en:"concrete", es:"hormigón"},
    {en:"condition", es:"condición"},
    {en:"conduct", es:"conducta"},
    {en:"conference", es:"conferencia"},
    {en:"confidence", es:"confianza"},
    {en:"confident", es:"seguro"},
    {en:"confirm", es:"confirmar"},
    {en:"conflict", es:"conflicto"},
    {en:"confront", es:"confrontar"},
    {en:"confusion", es:"Confusión"},
    {en:"Congress", es:"Congreso"},
    {en:"congressional", es:"del congreso"},
    {en:"connect", es:"conectar"},
    {en:"connection", es:"conexión"},
    {en:"consciousness", es:"conciencia"},
    {en:"consensus", es:"consenso"},
    {en:"consequence", es:"consecuencia"},
    {en:"conservative", es:"conservador"},
    {en:"consider", es:"considerar"},
    {en:"considerable", es:"considerable"},
    {en:"consideration", es:"consideración"},
    {en:"consist", es:"consistir"},
    {en:"consistent", es:"consistente"},
    {en:"constant", es:"constante"},
    {en:"constantly", es:"constantemente"},
    {en:"constitute", es:"constituir"},
    {en:"constitutional", es:"constitucional"},
    {en:"construct", es:"construir"},
    {en:"construction", es:"construcción"},
    {en:"consultant", es:"consultor"},
    {en:"consume", es:"consumir"},
    {en:"consumer", es:"consumidor"},
    {en:"consumption", es:"consumo"},
    {en:"contact", es:"contacto"},
    {en:"contain", es:"Contiene"},
    {en:"container", es:"envase"},
    {en:"contemporary", es:"contemporáneo"},
    {en:"content", es:"contenido"},
    {en:"contest", es:"concurso"},
    {en:"context", es:"contexto"},
    {en:"continue", es:"Seguir"},
    {en:"continued", es:"continuado"},
    {en:"contract", es:"contrato"},
    {en:"contrast", es:"contraste"},
    {en:"contribute", es:"contribuir"},
    {en:"contribution", es:"contribución"},
    {en:"control", es:"control"},
    {en:"controversial", es:"polémico"},
    {en:"controversy", es:"controversia"},
    {en:"convention", es:"convención"},
    {en:"conventional", es:"convencional"},
    {en:"conversation", es:"conversacion"},
    {en:"convert", es:"convertir"},
    {en:"conviction", es:"convicción"},
    {en:"convince", es:"convencer"},
    {en:"cook", es:"cocinar"},
    {en:"cookie", es:"Galleta"},
    {en:"cooking", es:"Cocinando"},
    {en:"cool", es:"frio"},
    {en:"cooperation", es:"cooperación"},
    {en:"cop", es:"policía"},
    {en:"cope", es:"afrontar"},
    {en:"copy", es:"Copiar"},
    {en:"core", es:"centro"},
    {en:"corn", es:"maíz"},
    {en:"corner", es:"esquina"},
    {en:"corporate", es:"corporativo"},
    {en:"corporation", es:"corporación"},
    {en:"correct", es:"correcto"},
    {en:"correspondent", es:"corresponsal"},
    {en:"cost", es:"costo"},
    {en:"cotton", es:"algodón"},
    {en:"couch", es:"sofá"},
    {en:"could", es:"pudo"},
    {en:"council", es:"Consejo"},
    {en:"counselor", es:"consejero"},
    {en:"count", es:"contar"},
    {en:"counter", es:"encimera"},
    {en:"country", es:"país"},
    {en:"county", es:"condado"},
    {en:"couple", es:"pareja"},
    {en:"courage", es:"coraje"},
    {en:"course", es:"curso"},
    {en:"court", es:"Corte"},
    {en:"cousin", es:"prima"},
    {en:"cover", es:"cubrir"},
    {en:"coverage", es:"cobertura"},
    {en:"cow", es:"vaca"},
    {en:"crack", es:"grieta"},
    {en:"craft", es:"artesanía"},
    {en:"crash", es:"choque"},
    {en:"crazy", es:"loco"},
    {en:"cream", es:"crema"},
    {en:"create", es:"crear"},
    {en:"creation", es:"creación"},
    {en:"creative", es:"creativo"},
    {en:"creature", es:"criatura"},
    {en:"credit", es:"crédito"},
    {en:"crew", es:"tripulación"},
    {en:"crime", es:"crimen"},
    {en:"criminal", es:"delincuente"},
    {en:"crisis", es:"crisis"},
    {en:"criteria", es:"Criterios"},
    {en:"critic", es:"crítico"},
    {en:"critical", es:"crítico"},
    {en:"criticism", es:"crítica"},
    {en:"criticize", es:"criticar"},
    {en:"crop", es:"cultivo"},
    {en:"cross", es:"cruzar"},
    {en:"crowd", es:"multitud"},
    {en:"crucial", es:"crucial"},
    {en:"cry", es:"llorar"},
    {en:"cultural", es:"cultural"},
    {en:"culture", es:"cultura"},
    {en:"cup", es:"taza"},
    {en:"curious", es:"curioso"},
    {en:"current", es:"Actual"},
    {en:"currently", es:"actualmente"},
    {en:"curriculum", es:"plan de estudios"},
    {en:"custom", es:"personalizado"},
    {en:"customer", es:"cliente"},
    {en:"cut", es:"cortar"},
    {en:"cycle", es:"ciclo"},
    {en:"dad", es:"padre"},
    {en:"daily", es:"a diario"},
    {en:"damage", es:"daño"},
    {en:"dance", es:"danza"},
    {en:"danger", es:"peligro"},
    {en:"dangerous", es:"peligroso"},
    {en:"dare", es:"atrevimiento"},
    {en:"dark", es:"oscuro"},
    {en:"darkness", es:"oscuridad"},
    {en:"data", es:"datos"},
    {en:"date", es:"fecha"},
    {en:"daughter", es:"hija"},
    {en:"day", es:"día"},
    {en:"dead", es:"muerto"},
    {en:"deal", es:"acuerdo"},
    {en:"dealer", es:"distribuidor"},
    {en:"dear", es:"querido"},
    {en:"death", es:"muerte"},
    {en:"debate", es:"debate"},
    {en:"debt", es:"deuda"},
    {en:"decade", es:"década"},
    {en:"decide", es:"decidir"},
    {en:"decision", es:"decisión"},
    {en:"deck", es:"plataforma"},
    {en:"declare", es:"declarar"},
    {en:"decline", es:"disminución"},
    {en:"decrease", es:"disminución"},
    {en:"deep", es:"profundo"},
    {en:"deeply", es:"profundamente"},
    {en:"deer", es:"ciervo"},
    {en:"defeat", es:"fracaso"},
    {en:"defend", es:"defender"},
    {en:"defendant", es:"acusado"},
    {en:"defense", es:"defensa"},
    {en:"defensive", es:"defensivo"},
    {en:"deficit", es:"déficit"},
    {en:"define", es:"definir"},
    {en:"definitely", es:"definitivamente"},
    {en:"definition", es:"definición"},
    {en:"degree", es:"la licenciatura"},
    {en:"delay", es:"demora"},
    {en:"deliver", es:"entregar"},
    {en:"delivery", es:"entrega"},
    {en:"demand", es:"demanda"},
    {en:"democracy", es:"democracia"},
    {en:"Democrat", es:"Demócrata"},
    {en:"democratic", es:"democrático"},
    {en:"demonstrate", es:"demostrar"},
    {en:"demonstration", es:"demostración"},
    {en:"deny", es:"negar"},
    {en:"department", es:"Departamento"},
    {en:"depend", es:"depender"},
    {en:"dependent", es:"dependiente"},
    {en:"depending", es:"dependiente"},
    {en:"depict", es:"representar"},
    {en:"depression", es:"depresión"},
    {en:"depth", es:"profundidad"},
    {en:"deputy", es:"diputado"},
    {en:"derive", es:"derivar"},
    {en:"describe", es:"describir"},
    {en:"description", es:"descripción"},
    {en:"desert", es:"Desierto"},
    {en:"deserve", es:"merecer"},
    {en:"design", es:"diseño"},
    {en:"designer", es:"diseñador"},
    {en:"desire", es:"deseo"},
    {en:"desk", es:"escritorio"},
    {en:"desperate", es:"desesperado"},
    {en:"despite", es:"A pesar de"},
    {en:"destroy", es:"destruir"},
    {en:"destruction", es:"destrucción"},
    {en:"detail", es:"detalle"},
    {en:"detailed", es:"detallado"},
    {en:"detect", es:"detectar"},
    {en:"determine", es:"determinar"},
    {en:"develop", es:"desarrollar"},
    {en:"developing", es:"desarrollando"},
    {en:"development", es:"desarrollo"},
    {en:"device", es:"dispositivo"},
    {en:"devote", es:"dedicar"},
    {en:"dialogue", es:"diálogo"},
    {en:"die", es:"morir"},
    {en:"diet", es:"dieta"},
    {en:"differ", es:"diferir de"},
    {en:"difference", es:"diferencia"},
    {en:"different", es:"diferente"},
    {en:"differently", es:"diferentemente"},
    {en:"difficult", es:"difícil"},
    {en:"difficulty", es:"dificultad"},
    {en:"dig", es:"cavar"},
    {en:"digital", es:"digital"},
    {en:"dimension", es:"dimensión"},
    {en:"dining", es:"comida"},
    {en:"dinner", es:"cena"},
    {en:"direct", es:"directo"},
    {en:"direction", es:"dirección"},
    {en:"directly", es:"directamente"},
    {en:"director", es:"director"},
    {en:"dirt", es:"suciedad"},
    {en:"dirty", es:"sucio"},
    {en:"disability", es:"discapacidad"},
    {en:"disagree", es:"discrepar"},
    {en:"disappear", es:"desaparecer"},
    {en:"disaster", es:"desastre"},
    {en:"discipline", es:"disciplina"},
    {en:"discourse", es:"discurso"},
    {en:"discover", es:"descubrir"},
    {en:"discovery", es:"descubrimiento"},
    {en:"discrimination", es:"discriminación"},
    {en:"discuss", es:"discutir"},
    {en:"discussion", es:"discusión"},
    {en:"disease", es:"enfermedad"},
    {en:"dish", es:"plato"},
    {en:"dismiss", es:"despedir"},
    {en:"disorder", es:"trastorno"},
    {en:"display", es:"monitor"},
    {en:"dispute", es:"disputa"},
    {en:"distance", es:"distancia"},
    {en:"distant", es:"distante"},
    {en:"distinct", es:"distinto"},
    {en:"distinction", es:"distinción"},
    {en:"distinguish", es:"distinguir"},
    {en:"distribute", es:"distribuir"},
    {en:"distribution", es:"distribución"},
    {en:"district", es:"distrito"},
    {en:"diverse", es:"diverso"},
    {en:"diversity", es:"diversidad"},
    {en:"divide", es:"dividir"},
    {en:"division", es:"división"},
    {en:"divorce", es:"divorcio"},
    {en:"DNA", es:"ADN"},
    {en:"do", es:"hacer"},
    {en:"doctor", es:"médico"},
    {en:"document", es:"documento"},
    {en:"dog", es:"perro"},
    {en:"domestic", es:"Doméstico"},
    {en:"dominant", es:"dominante"},
    {en:"dominate", es:"dominar"},
    {en:"door", es:"puerta"},
    {en:"double", es:"doble"},
    {en:"doubt", es:"duda"},
    {en:"down", es:"abajo"},
    {en:"downtown", es:"centro"},
    {en:"dozen", es:"docena"},
    {en:"draft", es:"sequía"},
    {en:"drag", es:"arrastrar"},
    {en:"drama", es:"drama"},
    {en:"dramatic", es:"dramático"},
    {en:"dramatically", es:"dramáticamente"},
    {en:"draw", es:"dibujar"},
    {en:"drawing", es:"dibujo"},
    {en:"dream", es:"sueño"},
    {en:"dress", es:"vestir"},
    {en:"drink", es:"beber"},
    {en:"drive", es:"conducir"},
    {en:"driver", es:"conductor"},
    {en:"drop", es:"soltar"},
    {en:"drug", es:"droga"},
    {en:"dry", es:"seco"},
    {en:"due", es:"vencer"},
    {en:"during", es:"durante"},
    {en:"dust", es:"polvo"},
    {en:"duty", es:"deber"},
    {en:"each", es:"cada"},
    {en:"eager", es:"entusiasta"},
    {en:"ear", es:"oreja"},
    {en:"early", es:"temprano"},
    {en:"earn", es:"ganar"},
    {en:"earnings", es:"ganancias"},
    {en:"earth", es:"tierra"},
    {en:"ease", es:"facilidad"},
    {en:"easily", es:"fácilmente"},
    {en:"east", es:"este"},
    {en:"eastern", es:"oriental"},
    {en:"easy", es:"fácil"},
    {en:"eat", es:"come"},
    {en:"economic", es:"económico"},
    {en:"economics", es:"ciencias económicas"},
    {en:"economist", es:"economista"},
    {en:"economy", es:"economía"},
    {en:"edge", es:"borde"},
    {en:"edition", es:"edición"},
    {en:"editor", es:"editor"},
    {en:"educate", es:"educar"},
    {en:"education", es:"educación"},
    {en:"educational", es:"educativo"},
    {en:"educator", es:"educador"},
    {en:"effect", es:"efecto"},
    {en:"effective", es:"eficaz"},
    {en:"effectively", es:"efectivamente"},
    {en:"efficiency", es:"eficiencia"},
    {en:"efficient", es:"eficiente"},
    {en:"effort", es:"esfuerzo"},
    {en:"egg", es:"huevo"},
    {en:"eight", es:"ocho"},
    {en:"either", es:"cualquiera"},
    {en:"elderly", es:"anciano"},
    {en:"elect", es:"electo"},
    {en:"election", es:"elección"},
    {en:"electric", es:"eléctrico"},
    {en:"electricity", es:"electricidad"},
    {en:"electronic", es:"electrónico"},
    {en:"element", es:"elemento"},
    {en:"elementary", es:"elemental"},
    {en:"eliminate", es:"eliminar"},
    {en:"elite", es:"élite"},
    {en:"else", es:"demás"},
    {en:"elsewhere", es:"en otra parte"},
    {en:"e-mail", es:"Email"},
    {en:"embrace", es:"abarcar"},
    {en:"emerge", es:"surgir"},
    {en:"emergency", es:"emergencia"},
    {en:"emission", es:"emisión"},
    {en:"emotion", es:"emoción"},
    {en:"emotional", es:"emocional"},
    {en:"emphasis", es:"énfasis"},
    {en:"emphasize", es:"enfatizar"},
    {en:"employ", es:"emplear"},
    {en:"employee", es:"empleado"},
    {en:"employer", es:"empleador"},
    {en:"employment", es:"empleo"},
    {en:"empty", es:"vacío"},
    {en:"enable", es:"permitir"},
    {en:"encounter", es:"encuentro"},
    {en:"encourage", es:"animar"},
    {en:"end", es:"fin"},
    {en:"enemy", es:"enemigo"},
    {en:"energy", es:"energía"},
    {en:"enforcement", es:"aplicación"},
    {en:"engage", es:"comprometerse"},
    {en:"engine", es:"motor"},
    {en:"engineer", es:"ingeniero"},
    {en:"engineering", es:"Ingeniería"},
    {en:"English", es:"inglés"},
    {en:"enhance", es:"mejorar"},
    {en:"enjoy", es:"disfrutar"},
    {en:"enormous", es:"enorme"},
    {en:"enough", es:"suficiente"},
    {en:"ensure", es:"garantizar"},
    {en:"enter", es:"ingresar"},
    {en:"enterprise", es:"empresa"},
    {en:"entertainment", es:"entretenimiento"},
    {en:"entire", es:"completo"},
    {en:"entirely", es:"enteramente"},
    {en:"entrance", es:"Entrada"},
    {en:"entry", es:"entrada"},
    {en:"environment", es:"ambiente"},
    {en:"environmental", es:"ambiental"},
    {en:"episode", es:"episodio"},
    {en:"equal", es:"igual"},
    {en:"equally", es:"igualmente"},
    {en:"equipment", es:"equipo"},
    {en:"era", es:"era"},
    {en:"error", es:"error"},
    {en:"escape", es:"escapar"},
    {en:"especially", es:"especialmente"},
    {en:"essay", es:"ensayo"},
    {en:"essential", es:"esencial"},
    {en:"essentially", es:"esencialmente"},
    {en:"establish", es:"establecer"},
    {en:"establishment", es:"establecimiento"},
    {en:"estate", es:"inmuebles"},
    {en:"estimate", es:"estimar"},
    {en:"etc", es:"etc"},
    {en:"ethics", es:"ética"},
    {en:"ethnic", es:"étnico"},
    {en:"European", es:"europeo"},
    {en:"evaluate", es:"evaluar"},
    {en:"evaluation", es:"evaluación"},
    {en:"even", es:"incluso"},
    {en:"evening", es:"noche"},
    {en:"event", es:"evento"},
    {en:"eventually", es:"finalmente"},
    {en:"ever", es:"alguna vez"},
    {en:"every", es:"cada"},
    {en:"everybody", es:"todos"},
    {en:"everyday", es:"cotidiano"},
    {en:"everyone", es:"todos"},
    {en:"everything", es:"todo"},
    {en:"everywhere", es:"En todas partes"},
    {en:"evidence", es:"evidencia"},
    {en:"evolution", es:"evolución"},
    {en:"evolve", es:"evolucionar"},
    {en:"exact", es:"exacto"},
    {en:"exactly", es:"exactamente"},
    {en:"examination", es:"examen"},
    {en:"examine", es:"examinar"},
    {en:"example", es:"ejemplo"},
    {en:"exceed", es:"exceder"},
    {en:"excellent", es:"excelente"},
    {en:"except", es:"excepto"},
    {en:"exception", es:"excepción"},
    {en:"exchange", es:"intercambio"},
    {en:"exciting", es:"excitante"},
    {en:"executive", es:"ejecutivo"},
    {en:"exercise", es:"ejercicio"},
    {en:"exhibit", es:"exposición"},
    {en:"exhibition", es:"exposición"},
    {en:"exist", es:"existe"},
    {en:"existence", es:"existencia"},
    {en:"existing", es:"existente"},
    {en:"expand", es:"expandir"},
    {en:"expansion", es:"expansión"},
    {en:"expect", es:"suponer"},
    {en:"expectation", es:"expectativa"},
    {en:"expense", es:"gastos"},
    {en:"expensive", es:"caro"},
    {en:"experience", es:"experiencia"},
    {en:"experiment", es:"experimentar"},
    {en:"expert", es:"experto"},
    {en:"explain", es:"explique"},
    {en:"explanation", es:"explicación"},
    {en:"explode", es:"explotar"},
    {en:"explore", es:"explorar"},
    {en:"explosion", es:"explosión"},
    {en:"expose", es:"exponer"},
    {en:"exposure", es:"exposición"},
    {en:"express", es:"Rápido"},
    {en:"expression", es:"expresión"},
    {en:"extend", es:"ampliar"},
    {en:"extension", es:"extensión"},
    {en:"extensive", es:"extenso"},
    {en:"extent", es:"grado"},
    {en:"external", es:"externo"},
    {en:"extra", es:"extra"},
    {en:"extraordinary", es:"extraordinario"},
    {en:"extreme", es:"extremo"},
    {en:"extremely", es:"extremadamente"},
    {en:"eye", es:"ojo"},
    {en:"fabric", es:"tela"},
    {en:"face", es:"rostro"},
    {en:"facility", es:"instalaciones"},
    {en:"fact", es:"hecho"},
    {en:"factor", es:"factor"},
    {en:"factory", es:"fábrica"},
    {en:"faculty", es:"facultad"},
    {en:"fade", es:"desvanecerse"},
    {en:"fail", es:"fallar"},
    {en:"failure", es:"falla"},
    {en:"fair", es:"justo"},
    {en:"fairly", es:"equitativamente"},
    {en:"faith", es:"fe"},
    {en:"fall", es:"caer"},
    {en:"false", es:"falso"},
    {en:"familiar", es:"familiar"},
    {en:"family", es:"familia"},
    {en:"famous", es:"famoso"},
    {en:"fan", es:"ventilador"},
    {en:"fantasy", es:"fantasía"},
    {en:"far", es:"lejos"},
    {en:"farm", es:"granja"},
    {en:"farmer", es:"granjero"},
    {en:"fashion", es:"Moda"},
    {en:"fast", es:"rápido"},
    {en:"fat", es:"gordo"},
    {en:"fate", es:"destino"},
    {en:"father", es:"padre"},
    {en:"fault", es:"culpa"},
    {en:"favor", es:"favor"},
    {en:"favorite", es:"favorito"},
    {en:"fear", es:"miedo"},
    {en:"feature", es:"característica"},
    {en:"federal", es:"federal"},
    {en:"fee", es:"tarifa"},
    {en:"feed", es:"alimentación"},
    {en:"feel", es:"sentir"},
    {en:"feeling", es:"sentimiento"},
    {en:"fellow", es:"compañero"},
    {en:"female", es:"mujer"},
    {en:"fence", es:"valla"},
    {en:"few", es:"pocos"},
    {en:"fewer", es:"menos"},
    {en:"fiber", es:"fibra"},
    {en:"fiction", es:"ficción"},
    {en:"field", es:"campo"},
    {en:"fifteen", es:"quince"},
    {en:"fifth", es:"quinto"},
    {en:"fifty", es:"cincuenta"},
    {en:"fight", es:"pelear"},
    {en:"fighter", es:"combatiente"},
    {en:"fighting", es:"lucha"},
    {en:"figure", es:"figura"},
    {en:"file", es:"Archivo"},
    {en:"fill", es:"llenar"},
    {en:"film", es:"película"},
    {en:"final", es:"final"},
    {en:"finally", es:"por fin"},
    {en:"finance", es:"Finanzas"},
    {en:"financial", es:"financiero"},
    {en:"find", es:"encontrar"},
    {en:"finding", es:"hallazgo"},
    {en:"fine", es:"multa"},
    {en:"finger", es:"dedo"},
    {en:"finish", es:"terminar"},
    {en:"fire", es:"fuego"},
    {en:"firm", es:"firma"},
    {en:"first", es:"primero"},
    {en:"fish", es:"pescado"},
    {en:"fishing", es:"pesca"},
    {en:"fit", es:"encajar"},
    {en:"fitness", es:"aptitud física"},
    {en:"five", es:"cinco"},
    {en:"fix", es:"arreglar"},
    {en:"flag", es:"bandera"},
    {en:"flame", es:"fuego"},
    {en:"flat", es:"plano"},
    {en:"flavor", es:"sabor"},
    {en:"flee", es:"huir"},
    {en:"flesh", es:"carne"},
    {en:"flight", es:"vuelo"},
    {en:"float", es:"flotador"},
    {en:"floor", es:"piso"},
    {en:"flow", es:"fluir"},
    {en:"flower", es:"flor"},
    {en:"fly", es:"volar"},
    {en:"focus", es:"enfocar"},
    {en:"folk", es:"gente"},
    {en:"follow", es:"seguir"},
    {en:"following", es:"siguiente"},
    {en:"food", es:"comida"},
    {en:"foot", es:"pie"},
    {en:"football", es:"fútbol americano"},
    {en:"for", es:"por"},
    {en:"force", es:"fuerza"},
    {en:"foreign", es:"extranjero"},
    {en:"forest", es:"bosque"},
    {en:"forever", es:"Siempre"},
    {en:"forget", es:"olvidar"},
    {en:"form", es:"formulario"},
    {en:"formal", es:"formal"},
    {en:"formation", es:"formación"},
    {en:"former", es:"anterior"},
    {en:"formula", es:"fórmula"},
    {en:"forth", es:"adelante"},
    {en:"fortune", es:"fortuna"},
    {en:"forward", es:"adelante"},
    {en:"found", es:"fundar"},
    {en:"foundation", es:"Fundación"},
    {en:"founder", es:"fundador"},
    {en:"four", es:"cuatro"},
    {en:"fourth", es:"cuatro"},
    {en:"frame", es:"marco"},
    {en:"framework", es:"marco de referencia"},
    {en:"free", es:"gratis"},
    {en:"freedom", es:"libertad"},
    {en:"freeze", es:"congelar"},
    {en:"French", es:"francés"},
    {en:"frequency", es:"frecuencia"},
    {en:"frequent", es:"frecuente"},
    {en:"frequently", es:"frecuentemente"},
    {en:"fresh", es:"Fresco"},
    {en:"friend", es:"amigo"},
    {en:"friendly", es:"simpático"},
    {en:"friendship", es:"amistad"},
    {en:"from", es:"desde"},
    {en:"front", es:"parte delantera"},
    {en:"fruit", es:"Fruta"},
    {en:"frustration", es:"frustración"},
    {en:"fuel", es:"combustible"},
    {en:"full", es:"completo"},
    {en:"fully", es:"completamente"},
    {en:"fun", es:"divertida"},
    {en:"function", es:"función"},
    {en:"fund", es:"fondo"},
    {en:"fundamental", es:"fundamental"},
    {en:"funding", es:"fondos"},
    {en:"funeral", es:"funeral"},
    {en:"funny", es:"gracioso"},
    {en:"furniture", es:"muebles"},
    {en:"furthermore", es:"además"},
    {en:"future", es:"futuro"},
    {en:"gain", es:"ganar"},
    {en:"galaxy", es:"galaxia"},
    {en:"gallery", es:"galería"},
    {en:"game", es:"juego"},
    {en:"gang", es:"pandilla"},
    {en:"gap", es:"brecha"},
    {en:"garage", es:"garaje"},
    {en:"garden", es:"jardín"},
    {en:"garlic", es:"ajo"},
    {en:"gas", es:"gas"},
    {en:"gate", es:"portón"},
    {en:"gather", es:"recolectar"},
    {en:"gay", es:"gay"},
    {en:"gaze", es:"mirada"},
    {en:"gear", es:"engranaje"},
    {en:"gender", es:"género"},
    {en:"gene", es:"gene"},
    {en:"general", es:"general"},
    {en:"generally", es:"en general"},
    {en:"generate", es:"generar"},
    {en:"generation", es:"Generacion"},
    {en:"genetic", es:"genético"},
    {en:"gentleman", es:"Caballero"},
    {en:"gently", es:"suavemente"},
    {en:"German", es:"alemán"},
    {en:"gesture", es:"gesto"},
    {en:"get", es:"obtener"},
    {en:"ghost", es:"fantasma"},
    {en:"giant", es:"gigante"},
    {en:"gift", es:"regalo"},
    {en:"gifted", es:"dotado"},
    {en:"girl", es:"muchacha"},
    {en:"girlfriend", es:"Novia"},
    {en:"give", es:"dar"},
    {en:"given", es:"dado"},
    {en:"glad", es:"contento"},
    {en:"glance", es:"mirada"},
    {en:"glass", es:"vidrio"},
    {en:"global", es:"global"},
    {en:"glove", es:"guante"},
    {en:"go", es:"Vamos"},
    {en:"goal", es:"meta"},
    {en:"God", es:"Dios"},
    {en:"gold", es:"oro"},
    {en:"golden", es:"dorado"},
    {en:"golf", es:"golf"},
    {en:"good", es:"bien"},
    {en:"government", es:"Gobierno"},
    {en:"governor", es:"gobernador"},
    {en:"grab", es:"tomar"},
    {en:"grade", es:"calificación"},
    {en:"gradually", es:"gradualmente"},
    {en:"graduate", es:"graduado"},
    {en:"grain", es:"grano"},
    {en:"grand", es:"grandioso"},
    {en:"grandfather", es:"abuelo"},
    {en:"grandmother", es:"abuela"},
    {en:"grant", es:"conceder"},
    {en:"grass", es:"césped"},
    {en:"grave", es:"tumba"},
    {en:"gray", es:"gris"},
    {en:"great", es:"estupendo"},
    {en:"greatest", es:"mayor"},
    {en:"green", es:"verde"},
    {en:"grocery", es:"tienda de comestibles"},
    {en:"ground", es:"tierra"},
    {en:"group", es:"grupo"},
    {en:"grow", es:"crecer"},
    {en:"growing", es:"creciente"},
    {en:"growth", es:"crecimiento"},
    {en:"guarantee", es:"garantía"},
    {en:"guard", es:"Guardia"},
    {en:"guess", es:"adivinar"},
    {en:"guest", es:"invitado"},
    {en:"guide", es:"guía"},
    {en:"guideline", es:"guía"},
    {en:"guilty", es:"culpable"},
    {en:"gun", es:"pistola"},
    {en:"guy", es:"chico"},
    {en:"habit", es:"hábito"},
    {en:"habitat", es:"habitat"},
    {en:"hair", es:"cabello"},
    {en:"half", es:"mitad"},
    {en:"hall", es:"salón"},
    {en:"hand", es:"mano"},
    {en:"handful", es:"puñado"},
    {en:"handle", es:"resolver"},
    {en:"hang", es:"colgar"},
    {en:"happen", es:"ocurrir"},
    {en:"happy", es:"feliz"},
    {en:"hard", es:"duro"},
    {en:"hardly", es:"difícilmente"},
    {en:"hat", es:"sombrero"},
    {en:"hate", es:"odio"},
    {en:"have", es:"tener"},
    {en:"he", es:"él"},
    {en:"head", es:"cabeza"},
    {en:"headline", es:"titular"},
    {en:"headquarters", es:"sede"},
    {en:"health", es:"salud"},
    {en:"healthy", es:"sano"},
    {en:"hear", es:"escuchar"},
    {en:"hearing", es:"audiencia"},
    {en:"heart", es:"corazón"},
    {en:"heat", es:"calor"},
    {en:"heaven", es:"cielo"},
    {en:"heavily", es:"fuertemente"},
    {en:"heavy", es:"pesado"},
    {en:"heel", es:"tacón"},
    {en:"height", es:"altura"},
    {en:"helicopter", es:"helicóptero"},
    {en:"hell", es:"infierno"},
    {en:"hello", es:"Hola"},
    {en:"help", es:"ayuda"},
    {en:"helpful", es:"servicial"},
    {en:"her", es:"ella"},
    {en:"here", es:"aquí"},
    {en:"heritage", es:"herencia"},
    {en:"hero", es:"héroe"},
    {en:"herself", es:"sí misma"},
    {en:"hey", es:"Oye"},
    {en:"hi", es:"Hola"},
    {en:"hide", es:"esconder"},
    {en:"high", es:"alto"},
    {en:"highlight", es:"destacar"},
    {en:"highly", es:"altamente"},
    {en:"highway", es:"carretera"},
    {en:"hill", es:"colina"},
    {en:"him", es:"él"},
    {en:"himself", es:"él mismo"},
    {en:"hip", es:"cadera"},
    {en:"hire", es:"contratar"},
    {en:"his", es:"su"},
    {en:"historian", es:"historiador"},
    {en:"historic", es:"histórico"},
    {en:"historical", es:"histórico"},
    {en:"history", es:"historia"},
    {en:"hit", es:"pegar"},
    {en:"hold", es:"sostener"},
    {en:"hole", es:"agujero"},
    {en:"holiday", es:"vacaciones"},
    {en:"holy", es:"santo"},
    {en:"home", es:"casa"},
    {en:"homeless", es:"Vagabundo"},
    {en:"honest", es:"honesto"},
    {en:"honey", es:"miel"},
    {en:"honor", es:"honor"},
    {en:"hope", es:"esperanza"},
    {en:"horizon", es:"horizonte"},
    {en:"horror", es:"horror"},
    {en:"horse", es:"caballo"},
    {en:"hospital", es:"hospital"},
    {en:"host", es:"anfitrión"},
    {en:"hot", es:"caliente"},
    {en:"hotel", es:"hotel"},
    {en:"hour", es:"hora"},
    {en:"house", es:"casa"},
    {en:"household", es:"familiar"},
    {en:"housing", es:"alojamiento"},
    {en:"how", es:"cómo"},
    {en:"however", es:"sin embargo"},
    {en:"huge", es:"enorme"},
    {en:"human", es:"humano"},
    {en:"humor", es:"humor"},
    {en:"hundred", es:"centenar"},
    {en:"hungry", es:"hambriento"},
    {en:"hunter", es:"cazador"},
    {en:"hunting", es:"caza"},
    {en:"hurt", es:"herir"},
    {en:"husband", es:"esposo"},
    {en:"hypothesis", es:"hipótesis"},
    {en:"I", es:"I"},
    {en:"ice", es:"hielo"},
    {en:"idea", es:"ocurrencia"},
    {en:"ideal", es:"ideal"},
    {en:"identification", es:"identificación"},
    {en:"identify", es:"identificar"},
    {en:"identity", es:"identidad"},
    {en:"ie", es:"es decir"},
    {en:"if", es:"Si"},
    {en:"ignore", es:"ignorar"},
    {en:"ill", es:"enfermo"},
    {en:"illegal", es:"ilegal"},
    {en:"illness", es:"enfermedad"},
    {en:"illustrate", es:"ilustrar"},
    {en:"image", es:"imagen"},
    {en:"imagination", es:"imaginación"},
    {en:"imagine", es:"imagina"},
    {en:"immediate", es:"inmediato"},
    {en:"immediately", es:"inmediatamente"},
    {en:"immigrant", es:"inmigrante"},
    {en:"immigration", es:"inmigración"},
    {en:"impact", es:"impacto"},
    {en:"implement", es:"implementar"},
    {en:"implication", es:"implicación"},
    {en:"imply", es:"implicar"},
    {en:"importance", es:"importancia"},
    {en:"important", es:"importante"},
    {en:"impose", es:"imponer"},
    {en:"impossible", es:"imposible"},
    {en:"impress", es:"impresionar"},
    {en:"impression", es:"impresión"},
    {en:"impressive", es:"impresionante"},
    {en:"improve", es:"mejorar"},
    {en:"improvement", es:"mejora"},
    {en:"in", es:"en"},
    {en:"incentive", es:"incentivo"},
    {en:"incident", es:"incidente"},
    {en:"include", es:"incluir"},
    {en:"including", es:"incluso"},
    {en:"income", es:"ingreso"},
    {en:"incorporate", es:"incorporar"},
    {en:"increase", es:"aumento"},
    {en:"increased", es:"aumentado"},
    {en:"increasing", es:"creciente"},
    {en:"increasingly", es:"cada vez más"},
    {en:"incredible", es:"increíble"},
    {en:"indeed", es:"Por supuesto"},
    {en:"independence", es:"independencia"},
    {en:"independent", es:"independiente"},
    {en:"index", es:"índice"},
    {en:"Indian", es:"indio"},
    {en:"indicate", es:"indicar"},
    {en:"indication", es:"indicación"},
    {en:"individual", es:"individual"},
    {en:"industrial", es:"industrial"},
    {en:"industry", es:"industria"},
    {en:"infant", es:"infantil"},
    {en:"infection", es:"infección"},
    {en:"inflation", es:"inflación"},
    {en:"influence", es:"influencia"},
    {en:"inform", es:"informar"},
    {en:"information", es:"información"},
    {en:"ingredient", es:"ingrediente"},
    {en:"initial", es:"inicial"},
    {en:"initially", es:"inicialmente"},
    {en:"initiative", es:"iniciativa"},
    {en:"injury", es:"lesión"},
    {en:"inner", es:"interno"},
    {en:"innocent", es:"inocente"},
    {en:"inquiry", es:"consulta"},
    {en:"inside", es:"dentro"},
    {en:"insight", es:"visión"},
    {en:"insist", es:"insistir"},
    {en:"inspire", es:"inspirar"},
    {en:"install", es:"Instalar en pc"},
    {en:"instance", es:"ejemplo"},
    {en:"instead", es:"en lugar de"},
    {en:"institution", es:"institución"},
    {en:"institutional", es:"institucional"},
    {en:"instruction", es:"instrucción"},
    {en:"instructor", es:"instructor"},
    {en:"instrument", es:"instrumento"},
    {en:"insurance", es:"seguro"},
    {en:"intellectual", es:"intelectual"},
    {en:"intelligence", es:"inteligencia"},
    {en:"intend", es:"intentar"},
    {en:"intense", es:"intenso"},
    {en:"intensity", es:"intensidad"},
    {en:"intention", es:"intención"},
    {en:"interaction", es:"Interacción"},
    {en:"interest", es:"interesar"},
    {en:"interested", es:"interesado"},
    {en:"interesting", es:"interesante"},
    {en:"internal", es:"interno"},
    {en:"international", es:"internacional"},
    {en:"Internet", es:"Internet"},
    {en:"interpret", es:"interpretar"},
    {en:"interpretation", es:"interpretación"},
    {en:"intervention", es:"intervención"},
    {en:"interview", es:"entrevista"},
    {en:"into", es:"en"},
    {en:"introduce", es:"introducir"},
    {en:"introduction", es:"Introducción"},
    {en:"invasion", es:"invasión"},
    {en:"invest", es:"invertir"},
    {en:"investigate", es:"investigar"},
    {en:"investigation", es:"investigación"},
    {en:"investigator", es:"investigador"},
    {en:"investment", es:"inversión"},
    {en:"investor", es:"inversor"},
    {en:"invite", es:"invitación"},
    {en:"involve", es:"involucrar"},
    {en:"involved", es:"involucrado"},
    {en:"involvement", es:"intervención"},
    {en:"Iraqi", es:"iraquí"},
    {en:"Irish", es:"irlandesa"},
    {en:"iron", es:"planchar"},
    {en:"Islamic", es:"islámico"},
    {en:"island", es:"isla"},
    {en:"Israeli", es:"israelí"},
    {en:"issue", es:"asunto"},
    {en:"it", es:"eso"},
    {en:"Italian", es:"italiano"},
    {en:"item", es:"ít"},
    {en:"its", es:"su"},
    {en:"itself", es:"sí mismo"},
    {en:"jacket", es:"chaqueta"},
    {en:"jail", es:"celda"},
    {en:"Japanese", es:"japonés"},
    {en:"jet", es:"chorro"},
    {en:"Jew", es:"judío"},
    {en:"Jewish", es:"judío"},
    {en:"job", es:"trabajo"},
    {en:"join", es:"entrar"},
    {en:"joint", es:"articulación"},
    {en:"joke", es:"broma"},
    {en:"journal", es:"diario"},
    {en:"journalist", es:"el periodista"},
    {en:"journey", es:"viaje"},
    {en:"joy", es:"alegría"},
    {en:"judge", es:"juez"},
    {en:"judgment", es:"juicio"},
    {en:"juice", es:"jugo"},
    {en:"jump", es:"salto"},
    {en:"junior", es:"júnior"},
    {en:"jury", es:"jurado"},
    {en:"just", es:"sólo"},
    {en:"justice", es:"justicia"},
    {en:"justify", es:"justificar"},
    {en:"keep", es:"mantenerse"},
    {en:"key", es:"llave"},
    {en:"kick", es:"patear"},
    {en:"kid", es:"niño"},
    {en:"kill", es:"matar"},
    {en:"killer", es:"asesino"},
    {en:"killing", es:"asesinato"},
    {en:"kind", es:"amable"},
    {en:"king", es:"Rey"},
    {en:"kiss", es:"Beso"},
    {en:"kitchen", es:"cocina"},
    {en:"knee", es:"rodilla"},
    {en:"knife", es:"cuchillo"},
    {en:"knock", es:"golpear"},
    {en:"know", es:"saber"},
    {en:"knowledge", es:"conocimiento"},
    {en:"lab", es:"laboratorio"},
    {en:"label", es:"etiqueta"},
    {en:"labor", es:"labor"},
    {en:"laboratory", es:"laboratorio"},
    {en:"lack", es:"ausencia"},
    {en:"lady", es:"señora"},
    {en:"lake", es:"lago"},
    {en:"land", es:"tierra"},
    {en:"landscape", es:"paisaje"},
    {en:"language", es:"idioma"},
    {en:"lap", es:"vuelta"},
    {en:"large", es:"grande"},
    {en:"largely", es:"en gran parte"},
    {en:"last", es:"ultimo"},
    {en:"late", es:"tarde"},
    {en:"later", es:"más tarde"},
    {en:"Latin", es:"latín"},
    {en:"latter", es:"último"},
    {en:"laugh", es:"reír"},
    {en:"launch", es:"lanzamiento"},
    {en:"law", es:"ley"},
    {en:"lawn", es:"césped"},
    {en:"lawsuit", es:"demanda judicial"},
    {en:"lawyer", es:"abogado"},
    {en:"lay", es:"poner"},
    {en:"layer", es:"capa"},
    {en:"lead", es:"dirigir"},
    {en:"leader", es:"líder"},
    {en:"leadership", es:"liderazgo"},
    {en:"leading", es:"principal"},
    {en:"leaf", es:"hoja"},
    {en:"league", es:"liga"},
    {en:"lean", es:"inclinarse"},
    {en:"learn", es:"aprender"},
    {en:"learning", es:"aprendiendo"},
    {en:"least", es:"menos"},
    {en:"leather", es:"cuero"},
    {en:"leave", es:"salir"},
    {en:"left", es:"izquierda"},
    {en:"leg", es:"pierna"},
    {en:"legacy", es:"legado"},
    {en:"legal", es:"legal"},
    {en:"legend", es:"leyenda"},
    {en:"legislation", es:"legislación"},
    {en:"legitimate", es:"legítimo"},
    {en:"lemon", es:"limón"},
    {en:"length", es:"longitud"},
    {en:"less", es:"menos"},
    {en:"lesson", es:"lección"},
    {en:"let", es:"dejar"},
    {en:"letter", es:"carta"},
    {en:"level", es:"nivel"},
    {en:"liberal", es:"liberal"},
    {en:"library", es:"Biblioteca"},
    {en:"license", es:"licencia"},
    {en:"lie", es:"mentir"},
    {en:"life", es:"vida"},
    {en:"lifestyle", es:"estilo de vida"},
    {en:"lifetime", es:"toda la vida"},
    {en:"lift", es:"elevar"},
    {en:"light", es:"ligero"},
    {en:"like", es:"me gusta"},
    {en:"likely", es:"probable"},
    {en:"limit", es:"límite"},
    {en:"limitation", es:"limitación"},
    {en:"limited", es:"limitado"},
    {en:"line", es:"línea"},
    {en:"link", es:"Enlace"},
    {en:"lip", es:"labio"},
    {en:"list", es:"lista"},
    {en:"listen", es:"escucha"},
    {en:"literally", es:"literalmente"},
    {en:"literary", es:"literario"},
    {en:"literature", es:"literatura"},
    {en:"little", es:"pequeño"},
    {en:"live", es:"En Vivo"},
    {en:"living", es:"viviendo"},
    {en:"load", es:"carga"},
    {en:"loan", es:"préstamo"},
    {en:"local", es:"local"},
    {en:"locate", es:"localizar"},
    {en:"location", es:"ubicación"},
    {en:"lock", es:"cerrar con llave"},
    {en:"long", es:"largo"},
    {en:"long-term", es:"a largo plazo"},
    {en:"look", es:"Mira"},
    {en:"loose", es:"suelto"},
    {en:"lose", es:"perder"},
    {en:"loss", es:"pérdida"},
    {en:"lost", es:"perdió"},
    {en:"lot", es:"lote"},
    {en:"lots", es:"lotes"},
    {en:"loud", es:"fuerte"},
    {en:"love", es:"amor"},
    {en:"lovely", es:"encantador"},
    {en:"lover", es:"amante"},
    {en:"low", es:"bajo"},
    {en:"lower", es:"más bajo"},
    {en:"luck", es:"suerte"},
    {en:"lucky", es:"suerte"},
    {en:"lunch", es:"comida"},
    {en:"lung", es:"pulmón"},
    {en:"machine", es:"máquina"},
    {en:"mad", es:"enojado"},
    {en:"magazine", es:"revista"},
    {en:"mail", es:"correo"},
    {en:"main", es:"principal"},
    {en:"mainly", es:"principalmente"},
    {en:"maintain", es:"mantener"},
    {en:"maintenance", es:"mantenimiento"},
    {en:"major", es:"importante"},
    {en:"majority", es:"mayoria"},
    {en:"make", es:"fabricar"},
    {en:"maker", es:"fabricante"},
    {en:"makeup", es:"maquillaje"},
    {en:"male", es:"masculino"},
    {en:"mall", es:"centro comercial"},
    {en:"man", es:"hombre"},
    {en:"manage", es:"gestionar"},
    {en:"management", es:"administración"},
    {en:"manager", es:"gerente"},
    {en:"manner", es:"manera"},
    {en:"manufacturer", es:"fabricante"},
    {en:"manufacturing", es:"fabricación"},
    {en:"many", es:"muchos"},
    {en:"map", es:"mapa"},
    {en:"margin", es:"margen"},
    {en:"mark", es:"Marcos"},
    {en:"market", es:"mercado"},
    {en:"marketing", es:"márketing"},
    {en:"marriage", es:"matrimonio"},
    {en:"married", es:"casado"},
    {en:"marry", es:"casar"},
    {en:"mask", es:"máscara"},
    {en:"mass", es:"masa"},
    {en:"massive", es:"masivo"},
    {en:"master", es:"Maestro"},
    {en:"match", es:"coincidir"},
    {en:"material", es:"material"},
    {en:"math", es:"Matemáticas"},
    {en:"matter", es:"importar"},
    {en:"may", es:"mayo"},
    {en:"maybe", es:"quizás"},
    {en:"mayor", es:"alcalde"},
    {en:"me", es:"yo"},
    {en:"meal", es:"comida"},
    {en:"mean", es:"significar"},
    {en:"meaning", es:"significado"},
    {en:"meanwhile", es:"mientras tanto"},
    {en:"measure", es:"la medida"},
    {en:"measurement", es:"medición"},
    {en:"meat", es:"carne"},
    {en:"mechanism", es:"mecanismo"},
    {en:"media", es:"medios de comunicación"},
    {en:"medical", es:"médico"},
    {en:"medication", es:"medicamento"},
    {en:"medicine", es:"medicamento"},
    {en:"medium", es:"medio"},
    {en:"meet", es:"reunirse"},
    {en:"meeting", es:"reunión"},
    {en:"member", es:"miembro"},
    {en:"membership", es:"afiliación"},
    {en:"memory", es:"memoria"},
    {en:"mental", es:"mental"},
    {en:"mention", es:"mencionar"},
    {en:"menu", es:"menú"},
    {en:"mere", es:"mero"},
    {en:"merely", es:"simplemente"},
    {en:"mess", es:"desorden"},
    {en:"message", es:"mensaje"},
    {en:"metal", es:"metal"},
    {en:"meter", es:"metro"},
    {en:"method", es:"método"},
    {en:"Mexican", es:"mexicano"},
    {en:"middle", es:"medio"},
    {en:"might", es:"podría"},
    {en:"military", es:"militar"},
    {en:"milk", es:"Leche"},
    {en:"million", es:"millón"},
    {en:"mind", es:"mente"},
    {en:"mine", es:"Mia"},
    {en:"minister", es:"ministro"},
    {en:"minor", es:"menor"},
    {en:"minority", es:"minoría"},
    {en:"minute", es:"minuto"},
    {en:"miracle", es:"milagro"},
    {en:"mirror", es:"espejo"},
    {en:"miss", es:"perder"},
    {en:"missile", es:"misil"},
    {en:"mission", es:"misión"},
    {en:"mistake", es:"Error"},
    {en:"mix", es:"mezcla"},
    {en:"mixture", es:"mezcla"},
    {en:"mode", es:"modo"},
    {en:"model", es:"modelo"},
    {en:"moderate", es:"moderar"},
    {en:"modern", es:"moderno"},
    {en:"modest", es:"modesto"},
    {en:"mom", es:"mamá"},
    {en:"moment", es:"momento"},
    {en:"money", es:"dinero"},
    {en:"monitor", es:"monitor"},
    {en:"month", es:"mes"},
    {en:"mood", es:"ánimo"},
    {en:"moon", es:"Luna"},
    {en:"moral", es:"moral"},
    {en:"more", es:"más"},
    {en:"moreover", es:"es más"},
    {en:"morning", es:"Mañana"},
    {en:"mortgage", es:"hipoteca"},
    {en:"most", es:"más"},
    {en:"mostly", es:"principalmente"},
    {en:"mother", es:"mamá"},
    {en:"motion", es:"movimiento"},
    {en:"motivation", es:"motivación"},
    {en:"motor", es:"motor"},
    {en:"mount", es:"montar"},
    {en:"mountain", es:"montaña"},
    {en:"mouse", es:"ratón"},
    {en:"mouth", es:"boca"},
    {en:"move", es:"Muevete"},
    {en:"movement", es:"movimiento"},
    {en:"movie", es:"película"},
    {en:"Mr", es:"Sres"},
    {en:"Mrs", es:"Señora"},
    {en:"Ms", es:"Em"},
    {en:"much", es:"mucho"},
    {en:"multiple", es:"múltiple"},
    {en:"murder", es:"asesinato"},
    {en:"muscle", es:"músculo"},
    {en:"museum", es:"museo"},
    {en:"music", es:"música"},
    {en:"musical", es:"musical"},
    {en:"musician", es:"músico"},
    {en:"Muslim", es:"musulmán"},
    {en:"must", es:"deber"},
    {en:"mutual", es:"mutuo"},
    {en:"my", es:"mi"},
    {en:"myself", es:"yo mismo"},
    {en:"mystery", es:"misterio"},
    {en:"myth", es:"mito"},
    {en:"naked", es:"desnudo"},
    {en:"name", es:"nombre"},
    {en:"narrative", es:"narrativa"},
    {en:"narrow", es:"estrecho"},
    {en:"nation", es:"nación"},
    {en:"national", es:"nacional"},
    {en:"native", es:"nativo"},
    {en:"natural", es:"natural"},
    {en:"naturally", es:"naturalmente"},
    {en:"nature", es:"naturaleza"},
    {en:"near", es:"cerca"},
    {en:"nearby", es:"cercano"},
    {en:"nearly", es:"por poco"},
    {en:"necessarily", es:"necesariamente"},
    {en:"necessary", es:"necesario"},
    {en:"neck", es:"cuello"},
    {en:"need", es:"necesitar"},
    {en:"negative", es:"negativo"},
    {en:"negotiate", es:"negociar"},
    {en:"negotiation", es:"negociación"},
    {en:"neighbor", es:"Vecino"},
    {en:"neighborhood", es:"vecindario"},
    {en:"neither", es:"ninguno de los dos"},
    {en:"nerve", es:"nervio"},
    {en:"nervous", es:"nervioso"},
    {en:"net", es:"red"},
    {en:"network", es:"red"},
    {en:"never", es:"nunca"},
    {en:"nevertheless", es:"sin embargo"},
    {en:"new", es:"nuevo"},
    {en:"newly", es:"recién"},
    {en:"news", es:"Noticias"},
    {en:"newspaper", es:"periódico"},
    {en:"next", es:"Siguiente"},
    {en:"nice", es:"bonito"},
    {en:"night", es:"noche"},
    {en:"nine", es:"nueve"},
    {en:"no", es:"No"},
    {en:"nobody", es:"nadie"},
    {en:"nod", es:"asentir"},
    {en:"noise", es:"ruido"},
    {en:"nomination", es:"nominación"},
    {en:"none", es:"ninguna"},
    {en:"nonetheless", es:"sin embargo"},
    {en:"nor", es:"ni"},
    {en:"normal", es:"normal"},
    {en:"normally", es:"normalmente"},
    {en:"north", es:"norte"},
    {en:"northern", es:"del Norte"},
    {en:"nose", es:"nariz"},
    {en:"not", es:"no"},
    {en:"note", es:"Nota"},
    {en:"nothing", es:"nada"},
    {en:"notice", es:"aviso"},
    {en:"notion", es:"noción"},
    {en:"novel", es:"novela"},
    {en:"now", es:"ahora"},
    {en:"nowhere", es:"en ningún lugar"},
    {en:"n't", es:"Nuevo Testamento"},
    {en:"nuclear", es:"nuclear"},
    {en:"number", es:"número"},
    {en:"numerous", es:"numeroso"},
    {en:"nurse", es:"enfermero"},
    {en:"nut", es:"tuerca"},
    {en:"object", es:"objeto"},
    {en:"objective", es:"objetivo"},
    {en:"obligation", es:"obligación"},
    {en:"observation", es:"observación"},
    {en:"observe", es:"observar"},
    {en:"observer", es:"observador"},
    {en:"obtain", es:"obtener"},
    {en:"obvious", es:"obvio"},
    {en:"obviously", es:"obviamente"},
    {en:"occasion", es:"ocasión"},
    {en:"occasionally", es:"de vez en cuando"},
    {en:"occupation", es:"ocupación"},
    {en:"occupy", es:"ocupar"},
    {en:"occur", es:"ocurrir"},
    {en:"ocean", es:"Oceano"},
    {en:"odd", es:"impar"},
    {en:"odds", es:"impares"},
    {en:"of", es:"de"},
    {en:"off", es:"apagado"},
    {en:"offense", es:"ofensa"},
    {en:"offensive", es:"ofensiva"},
    {en:"offer", es:"oferta"},
    {en:"office", es:"oficina"},
    {en:"officer", es:"oficial"},
    {en:"official", es:"oficial"},
    {en:"often", es:"a menudo"},
    {en:"oh", es:"Oh"},
    {en:"oil", es:"aceite"},
    {en:"ok", es:"OK"},
    {en:"okay", es:"bueno"},
    {en:"old", es:"viejo"},
    {en:"Olympic", es:"olímpico"},
    {en:"on", es:"sobre"},
    {en:"once", es:"una vez"},
    {en:"one", es:"una"},
    {en:"ongoing", es:"en curso"},
    {en:"onion", es:"cebolla"},
    {en:"online", es:"en línea"},
    {en:"only", es:"solo"},
    {en:"onto", es:"sobre"},
    {en:"open", es:"abierto"},
    {en:"opening", es:"apertura"},
    {en:"operate", es:"funcionar"},
    {en:"operating", es:"operando"},
    {en:"operation", es:"operación"},
    {en:"operator", es:"operador"},
    {en:"opinion", es:"opinión"},
    {en:"opponent", es:"adversario"},
    {en:"opportunity", es:"oportunidad"},
    {en:"oppose", es:"oponerse a"},
    {en:"opposite", es:"opuesto"},
    {en:"opposition", es:"oposición"},
    {en:"option", es:"opción"},
    {en:"or", es:"o"},
    {en:"orange", es:"naranja"},
    {en:"order", es:"pedido"},
    {en:"ordinary", es:"ordinario"},
    {en:"organic", es:"orgánico"},
    {en:"organization", es:"organización"},
    {en:"organize", es:"organizar"},
    {en:"orientation", es:"orientación"},
    {en:"origin", es:"origen"},
    {en:"original", es:"original"},
    {en:"originally", es:"originalmente"},
    {en:"other", es:"otro"},
    {en:"others", es:"otros"},
    {en:"otherwise", es:"de lo contrario"},
    {en:"ought", es:"debería"},
    {en:"our", es:"nuestro"},
    {en:"ourselves", es:"Nosotros mismos"},
    {en:"out", es:"fuera"},
    {en:"outcome", es:"Salir"},
    {en:"outside", es:"fuera de"},
    {en:"oven", es:"horno"},
    {en:"over", es:"encima"},
    {en:"overall", es:"general"},
    {en:"overcome", es:"superar"},
    {en:"overlook", es:"pasar por alto"},
    {en:"owe", es:"deber"},
    {en:"own", es:"propio"},
    {en:"owner", es:"dueño"},
    {en:"pace", es:"ritmo"},
    {en:"pack", es:"paquete"},
    {en:"package", es:"paquete"},
    {en:"page", es:"página"},
    {en:"pain", es:"dolor"},
    {en:"painful", es:"doloroso"},
    {en:"paint", es:"pintura"},
    {en:"painter", es:"pintor"},
    {en:"painting", es:"cuadro"},
    {en:"pair", es:"par"},
    {en:"pale", es:"pálido"},
    {en:"Palestinian", es:"palestino"},
    {en:"palm", es:"palma"},
    {en:"pan", es:"sartén"},
    {en:"panel", es:"panel"},
    {en:"pant", es:"pantalón"},
    {en:"paper", es:"papel"},
    {en:"parent", es:"padre"},
    {en:"park", es:"parque"},
    {en:"parking", es:"estacionamiento"},
    {en:"part", es:"parte"},
    {en:"participant", es:"partícipe"},
    {en:"participate", es:"participar"},
    {en:"participation", es:"participación"},
    {en:"particular", es:"especial"},
    {en:"particularly", es:"particularmente"},
    {en:"partly", es:"parcialmente"},
    {en:"partner", es:"compañero"},
    {en:"partnership", es:"camaradería"},
    {en:"party", es:"fiesta"},
    {en:"pass", es:"aprobar"},
    {en:"passage", es:"paso"},
    {en:"passenger", es:"pasajero"},
    {en:"passion", es:"pasión"},
    {en:"past", es:"pasado"},
    {en:"patch", es:"parche"},
    {en:"path", es:"sendero"},
    {en:"patient", es:"paciente"},
    {en:"pattern", es:"patrón"},
    {en:"pause", es:"pausa"},
    {en:"pay", es:"pagar"},
    {en:"payment", es:"pago"},
    {en:"PC", es:"ordenador personal"},
    {en:"peace", es:"paz"},
    {en:"peak", es:"cima"},
    {en:"peer", es:"mirar"},
    {en:"penalty", es:"multa"},
    {en:"people", es:"personas"},
    {en:"pepper", es:"pimienta"},
    {en:"per", es:"por"},
    {en:"perceive", es:"percibir"},
    {en:"percentage", es:"porcentaje"},
    {en:"perception", es:"percepción"},
    {en:"perfect", es:"Perfecto"},
    {en:"perfectly", es:"perfectamente"},
    {en:"perform", es:"llevar a cabo"},
    {en:"performance", es:"rendimiento"},
    {en:"perhaps", es:"quizás"},
    {en:"period", es:"período"},
    {en:"permanent", es:"permanente"},
    {en:"permission", es:"permiso"},
    {en:"permit", es:"permiso"},
    {en:"person", es:"persona"},
    {en:"personal", es:"personal"},
    {en:"personality", es:"personalidad"},
    {en:"personally", es:"personalmente"},
    {en:"personnel", es:"personal"},
    {en:"perspective", es:"perspectiva"},
    {en:"persuade", es:"persuadir"},
    {en:"pet", es:"mascota"},
    {en:"phase", es:"fase"},
    {en:"phenomenon", es:"fenómeno"},
    {en:"philosophy", es:"filosofía"},
    {en:"phone", es:"teléfono"},
    {en:"photo", es:"Foto"},
    {en:"photograph", es:"fotografía"},
    {en:"photographer", es:"fotógrafo"},
    {en:"phrase", es:"frase"},
    {en:"physical", es:"físico"},
    {en:"physically", es:"físicamente"},
    {en:"physician", es:"médico"},
    {en:"piano", es:"piano"},
    {en:"pick", es:"elegir"},
    {en:"picture", es:"imagen"},
    {en:"pie", es:"tarta"},
    {en:"piece", es:"pieza"},
    {en:"pile", es:"pila"},
    {en:"pilot", es:"piloto"},
    {en:"pine", es:"pino"},
    {en:"pink", es:"Rosa"},
    {en:"pipe", es:"tubo"},
    {en:"pitch", es:"terreno de juego"},
    {en:"place", es:"lugar"},
    {en:"plan", es:"plan"},
    {en:"plane", es:"avión"},
    {en:"planet", es:"planeta"},
    {en:"planning", es:"planificación"},
    {en:"plant", es:"planta"},
    {en:"plastic", es:"el plastico"},
    {en:"plate", es:"plato"},
    {en:"platform", es:"plataforma"},
    {en:"play", es:"tocar"},
    {en:"player", es:"jugador"},
    {en:"please", es:"por favor"},
    {en:"pleasure", es:"Placer"},
    {en:"plenty", es:"mucho"},
    {en:"plot", es:"gráfico"},
    {en:"plus", es:"más"},
    {en:"PM", es:"PM"},
    {en:"pocket", es:"bolsillo"},
    {en:"poem", es:"poema"},
    {en:"poet", es:"poeta"},
    {en:"poetry", es:"poesía"},
    {en:"point", es:"punto"},
    {en:"pole", es:"polo"},
    {en:"police", es:"policía"},
    {en:"policy", es:"política"},
    {en:"political", es:"político"},
    {en:"politically", es:"políticamente"},
    {en:"politician", es:"político"},
    {en:"politics", es:"política"},
    {en:"poll", es:"encuesta"},
    {en:"pollution", es:"polución"},
    {en:"pool", es:"piscina"},
    {en:"poor", es:"pobre"},
    {en:"pop", es:"música pop"},
    {en:"popular", es:"popular"},
    {en:"population", es:"población"},
    {en:"porch", es:"porche"},
    {en:"port", es:"Puerto"},
    {en:"portion", es:"parte"},
    {en:"portrait", es:"retrato"},
    {en:"portray", es:"retratar"},
    {en:"pose", es:"pose"},
    {en:"position", es:"posición"},
    {en:"positive", es:"positivo"},
    {en:"possess", es:"poseer"},
    {en:"possibility", es:"posibilidad"},
    {en:"possible", es:"posible"},
    {en:"possibly", es:"posiblemente"},
    {en:"post", es:"correo"},
    {en:"pot", es:"maceta"},
    {en:"potato", es:"Papa"},
    {en:"potential", es:"potencial"},
    {en:"potentially", es:"potencialmente"},
    {en:"pound", es:"libra"},
    {en:"pour", es:"verter"},
    {en:"poverty", es:"pobreza"},
    {en:"powder", es:"polvo"},
    {en:"power", es:"energía"},
    {en:"powerful", es:"poderoso"},
    {en:"practical", es:"práctico"},
    {en:"practice", es:"práctica"},
    {en:"pray", es:"rezar"},
    {en:"prayer", es:"oración"},
    {en:"precisely", es:"precisamente"},
    {en:"predict", es:"predecir"},
    {en:"prefer", es:"preferir"},
    {en:"preference", es:"preferencia"},
    {en:"pregnancy", es:"el embarazo"},
    {en:"pregnant", es:"embarazada"},
    {en:"preparation", es:"preparación"},
    {en:"prepare", es:"preparar"},
    {en:"prescription", es:"prescripción"},
    {en:"presence", es:"presencia"},
    {en:"present", es:"regalo"},
    {en:"presentation", es:"presentación"},
    {en:"preserve", es:"preservar"},
    {en:"president", es:"presidente"},
    {en:"presidential", es:"presidencial"},
    {en:"press", es:"imprenta"},
    {en:"pressure", es:"presión"},
    {en:"pretend", es:"fingir"},
    {en:"pretty", es:"bonito"},
    {en:"prevent", es:"evitar"},
    {en:"previous", es:"anterior"},
    {en:"previously", es:"previamente"},
    {en:"price", es:"precio"},
    {en:"pride", es:"orgullo"},
    {en:"priest", es:"sacerdote"},
    {en:"primarily", es:"ante todo"},
    {en:"primary", es:"primario"},
    {en:"prime", es:"principal"},
    {en:"principal", es:"principal"},
    {en:"principle", es:"principio"},
    {en:"print", es:"impresión"},
    {en:"prior", es:"previo"},
    {en:"priority", es:"prioridad"},
    {en:"prison", es:"prisión"},
    {en:"prisoner", es:"prisionero"},
    {en:"privacy", es:"intimidad"},
    {en:"private", es:"privado"},
    {en:"probably", es:"probablemente"},
    {en:"problem", es:"problema"},
    {en:"procedure", es:"procedimiento"},
    {en:"proceed", es:"Continuar"},
    {en:"process", es:"proceso"},
    {en:"produce", es:"Produce"},
    {en:"producer", es:"productor"},
    {en:"product", es:"producto"},
    {en:"production", es:"producción"},
    {en:"profession", es:"profesión"},
    {en:"professional", es:"profesional"},
    {en:"professor", es:"profesor"},
    {en:"profile", es:"perfil"},
    {en:"profit", es:"lucro"},
    {en:"program", es:"programa"},
    {en:"progress", es:"Progreso"},
    {en:"project", es:"proyecto"},
    {en:"prominent", es:"prominente"},
    {en:"promise", es:"promesa"},
    {en:"promote", es:"promover"},
    {en:"prompt", es:"inmediato"},
    {en:"proof", es:"prueba"},
    {en:"proper", es:"adecuado"},
    {en:"properly", es:"adecuadamente"},
    {en:"property", es:"propiedad"},
    {en:"proportion", es:"proporción"},
    {en:"proposal", es:"propuesta"},
    {en:"propose", es:"proponer"},
    {en:"proposed", es:"propuesto"},
    {en:"prosecutor", es:"fiscal"},
    {en:"prospect", es:"perspectiva"},
    {en:"protect", es:"proteger"},
    {en:"protection", es:"proteccion"},
    {en:"protein", es:"proteína"},
    {en:"protest", es:"protesta"},
    {en:"proud", es:"orgulloso"},
    {en:"prove", es:"probar"},
    {en:"provide", es:"proveer"},
    {en:"provider", es:"proveedor"},
    {en:"province", es:"provincia"},
    {en:"provision", es:"provisión"},
    {en:"psychological", es:"psicológico"},
    {en:"psychologist", es:"psicólogo"},
    {en:"psychology", es:"psicología"},
    {en:"public", es:"público"},
    {en:"publication", es:"publicación"},
    {en:"publicly", es:"en público"},
    {en:"publish", es:"publicar"},
    {en:"publisher", es:"editor"},
    {en:"pull", es:"jalar"},
    {en:"punishment", es:"castigo"},
    {en:"purchase", es:"compra"},
    {en:"pure", es:"puro"},
    {en:"purpose", es:"propósito"},
    {en:"pursue", es:"perseguir"},
    {en:"push", es:"empujar"},
    {en:"put", es:"poner"},
    {en:"qualify", es:"calificar"},
    {en:"quality", es:"calidad"},
    {en:"quarter", es:"cuarto"},
    {en:"quarterback", es:"jugador de ataque"},
    {en:"question", es:"pregunta"},
    {en:"quick", es:"rápido"},
    {en:"quickly", es:"con rapidez"},
    {en:"quiet", es:"tranquilo"},
    {en:"quietly", es:"en silencio"},
    {en:"quit", es:"renunciar"},
    {en:"quite", es:"bastante"},
    {en:"quote", es:"cita"},
    {en:"race", es:"raza"},
    {en:"racial", es:"racial"},
    {en:"radical", es:"radical"},
    {en:"radio", es:"radio"},
    {en:"rail", es:"carril"},
    {en:"rain", es:"lluvia"},
    {en:"raise", es:"aumentar"},
    {en:"range", es:"rango"},
    {en:"rank", es:"rango"},
    {en:"rapid", es:"rápido"},
    {en:"rapidly", es:"rápidamente"},
    {en:"rare", es:"raro"},
    {en:"rarely", es:"casi nunca"},
    {en:"rate", es:"calificar"},
    {en:"rather", es:"bastante"},
    {en:"rating", es:"clasificación"},
    {en:"ratio", es:"proporción"},
    {en:"raw", es:"crudo"},
    {en:"reach", es:"alcanzar"},
    {en:"react", es:"reaccionar"},
    {en:"reaction", es:"reacción"},
    {en:"read", es:"leer"},
    {en:"reader", es:"lector"},
    {en:"reading", es:"leyendo"},
    {en:"ready", es:"Listo"},
    {en:"real", es:"verdadero"},
    {en:"reality", es:"realidad"},
    {en:"realize", es:"darse cuenta de"},
    {en:"really", es:"De Verdad"},
    {en:"reason", es:"razón"},
    {en:"reasonable", es:"razonable"},
    {en:"recall", es:"recuerdo"},
    {en:"receive", es:"recibir"},
    {en:"recent", es:"reciente"},
    {en:"recently", es:"recientemente"},
    {en:"recipe", es:"receta"},
    {en:"recognition", es:"reconocimiento"},
    {en:"recognize", es:"reconocer"},
    {en:"recommend", es:"recomendar"},
    {en:"recommendation", es:"recomendación"},
    {en:"record", es:"registro"},
    {en:"recording", es:"grabación"},
    {en:"recover", es:"recuperar"},
    {en:"recovery", es:"recuperación"},
    {en:"recruit", es:"recluta"},
    {en:"red", es:"rojo"},
    {en:"reduce", es:"reducir"},
    {en:"reduction", es:"reducción"},
    {en:"refer", es:"referir"},
    {en:"reference", es:"referencia"},
    {en:"reflect", es:"reflejar"},
    {en:"reflection", es:"reflexión"},
    {en:"reform", es:"reforma"},
    {en:"refugee", es:"refugiado"},
    {en:"refuse", es:"negar"},
    {en:"regard", es:"respecto"},
    {en:"regarding", es:"con respecto a"},
    {en:"regardless", es:"independientemente"},
    {en:"regime", es:"régimen"},
    {en:"region", es:"región"},
    {en:"regional", es:"regional"},
    {en:"register", es:"Registrarse"},
    {en:"regular", es:"regular"},
    {en:"regularly", es:"regularmente"},
    {en:"regulate", es:"regular"},
    {en:"regulation", es:"regulación"},
    {en:"reinforce", es:"reforzarse"},
    {en:"reject", es:"rechazar"},
    {en:"relate", es:"relacionar"},
    {en:"relation", es:"relación"},
    {en:"relationship", es:"relación"},
    {en:"relative", es:"relativo"},
    {en:"relatively", es:"relativamente"},
    {en:"relax", es:"relajarse"},
    {en:"release", es:"liberar"},
    {en:"relevant", es:"pertinente"},
    {en:"relief", es:"alivio"},
    {en:"religion", es:"religión"},
    {en:"religious", es:"religioso"},
    {en:"rely", es:"confiar"},
    {en:"remain", es:"permanecer"},
    {en:"remaining", es:"restante"},
    {en:"remarkable", es:"Notable"},
    {en:"remember", es:"recordar"},
    {en:"remind", es:"recordar"},
    {en:"remote", es:"remoto"},
    {en:"remove", es:"retirar"},
    {en:"repeat", es:"repetir"},
    {en:"repeatedly", es:"repetidamente"},
    {en:"replace", es:"reemplazar"},
    {en:"reply", es:"respuesta"},
    {en:"report", es:"reporte"},
    {en:"reporter", es:"reportero"},
    {en:"represent", es:"representar"},
    {en:"representation", es:"representación"},
    {en:"representative", es:"representante"},
    {en:"Republican", es:"Republicano"},
    {en:"reputation", es:"reputación"},
    {en:"request", es:"solicitud"},
    {en:"require", es:"exigir"},
    {en:"requirement", es:"requisito"},
    {en:"research", es:"investigar"},
    {en:"researcher", es:"investigador"},
    {en:"resemble", es:"asemejarse a"},
    {en:"reservation", es:"reserva"},
    {en:"resident", es:"residente"},
    {en:"resist", es:"resistir"},
    {en:"resistance", es:"resistencia"},
    {en:"resolution", es:"resolución"},
    {en:"resolve", es:"resolver"},
    {en:"resort", es:"recurso"},
    {en:"resource", es:"recurso"},
    {en:"respect", es:"respeto"},
    {en:"respond", es:"responder"},
    {en:"respondent", es:"demandado"},
    {en:"response", es:"respuesta"},
    {en:"responsibility", es:"responsabilidad"},
    {en:"responsible", es:"responsable"},
    {en:"rest", es:"descansar"},
    {en:"restaurant", es:"restaurante"},
    {en:"restore", es:"restaurar"},
    {en:"restriction", es:"restricción"},
    {en:"result", es:"resultado"},
    {en:"retain", es:"conservar"},
    {en:"retire", es:"retirarse"},
    {en:"retirement", es:"Jubilación"},
    {en:"return", es:"regreso"},
    {en:"reveal", es:"revelar"},
    {en:"revenue", es:"ingresos"},
    {en:"review", es:"revisión"},
    {en:"revolution", es:"revolución"},
    {en:"rhythm", es:"ritmo"},
    {en:"rice", es:"arroz"},
    {en:"rich", es:"Rico"},
    {en:"rid", es:"deshacerse"},
    {en:"ride", es:"montar"},
    {en:"rifle", es:"rifle"},
    {en:"right", es:"derecho"},
    {en:"ring", es:"anillo"},
    {en:"rise", es:"aumento"},
    {en:"risk", es:"riesgo"},
    {en:"river", es:"río"},
    {en:"road", es:"camino"},
    {en:"rock", es:"rock"},
    {en:"role", es:"papel"},
    {en:"roll", es:"rollo"},
    {en:"romantic", es:"romántico"},
    {en:"roof", es:"techo"},
    {en:"room", es:"habitación"},
    {en:"root", es:"raíz"},
    {en:"rope", es:"soga"},
    {en:"rose", es:"Rosa"},
    {en:"rough", es:"áspero"},
    {en:"roughly", es:"aproximadamente"},
    {en:"round", es:"ronda"},
    {en:"route", es:"ruta"},
    {en:"routine", es:"rutina"},
    {en:"row", es:"hilera"},
    {en:"rub", es:"frotar"},
    {en:"rule", es:"regla"},
    {en:"run", es:"correr"},
    {en:"running", es:"corriendo"},
    {en:"rural", es:"rural"},
    {en:"rush", es:"prisa"},
    {en:"Russian", es:"ruso"},
    {en:"sacred", es:"sagrado"},
    {en:"sad", es:"triste"},
    {en:"safe", es:"a salvo"},
    {en:"safety", es:"seguridad"},
    {en:"sake", es:"motivo"},
    {en:"salad", es:"ensalada"},
    {en:"salary", es:"salario"},
    {en:"sale", es:"rebaja"},
    {en:"sales", es:"Ventas"},
    {en:"salt", es:"sal"},
    {en:"same", es:"mismo"},
    {en:"sample", es:"muestra"},
    {en:"sanction", es:"sanción"},
    {en:"sand", es:"arena"},
    {en:"satellite", es:"satélite"},
    {en:"satisfaction", es:"satisfacción"},
    {en:"satisfy", es:"satisfacer"},
    {en:"sauce", es:"salsa"},
    {en:"save", es:"ahorrar"},
    {en:"saving", es:"ahorro"},
    {en:"say", es:"decir"},
    {en:"scale", es:"escala"},
    {en:"scandal", es:"escándalo"},
    {en:"scared", es:"asustado"},
    {en:"scenario", es:"guión"},
    {en:"scene", es:"escena"},
    {en:"schedule", es:"calendario"},
    {en:"scheme", es:"esquema"},
    {en:"scholar", es:"erudito"},
    {en:"scholarship", es:"beca"},
    {en:"school", es:"colegio"},
    {en:"science", es:"Ciencias"},
    {en:"scientific", es:"científico"},
    {en:"scientist", es:"científico"},
    {en:"scope", es:"alcance"},
    {en:"score", es:"puntaje"},
    {en:"scream", es:"grito"},
    {en:"screen", es:"pantalla"},
    {en:"script", es:"texto"},
    {en:"sea", es:"mar"},
    {en:"search", es:"búsqueda"},
    {en:"season", es:"temporada"},
    {en:"seat", es:"asiento"},
    {en:"second", es:"segundo"},
    {en:"secret", es:"secreto"},
    {en:"secretary", es:"secretario"},
    {en:"section", es:"sección"},
    {en:"sector", es:"sector"},
    {en:"secure", es:"seguro"},
    {en:"security", es:"seguridad"},
    {en:"see", es:"ver"},
    {en:"seed", es:"semilla"},
    {en:"seek", es:"buscar"},
    {en:"seem", es:"parecer"},
    {en:"segment", es:"segmento"},
    {en:"seize", es:"aprovechar"},
    {en:"select", es:"Seleccione"},
    {en:"selection", es:"selección"},
    {en:"self", es:"uno mismo"},
    {en:"sell", es:"vender"},
    {en:"Senate", es:"Senado"},
    {en:"senator", es:"senador"},
    {en:"send", es:"enviar"},
    {en:"senior", es:"mayor"},
    {en:"sense", es:"sentido"},
    {en:"sensitive", es:"sensitivo"},
    {en:"sentence", es:"oración"},
    {en:"separate", es:"separar"},
    {en:"sequence", es:"secuencia"},
    {en:"series", es:"serie"},
    {en:"serious", es:"grave"},
    {en:"seriously", es:"seriamente"},
    {en:"serve", es:"atender"},
    {en:"service", es:"Servicio"},
    {en:"session", es:"sesión"},
    {en:"set", es:"colocar"},
    {en:"setting", es:"ajuste"},
    {en:"settle", es:"asentar"},
    {en:"settlement", es:"asentamiento"},
    {en:"seven", es:"Siete"},
    {en:"several", es:"varios"},
    {en:"severe", es:"grave"},
    {en:"sex", es:"sexo"},
    {en:"sexual", es:"sexual"},
    {en:"shade", es:"sombra"},
    {en:"shadow", es:"sombra"},
    {en:"shake", es:"sacudir"},
    {en:"shall", es:"deberá"},
    {en:"shape", es:"forma"},
    {en:"share", es:"Cuota"},
    {en:"sharp", es:"afilado"},
    {en:"she", es:"ella"},
    {en:"sheet", es:"hoja"},
    {en:"shelf", es:"estante"},
    {en:"shell", es:"cáscara"},
    {en:"shelter", es:"abrigo"},
    {en:"shift", es:"cambio"},
    {en:"shine", es:"brillar"},
    {en:"ship", es:"Embarcacion"},
    {en:"shirt", es:"camisa"},
    {en:"shit", es:"mierda"},
    {en:"shock", es:"conmoción"},
    {en:"shoe", es:"zapato"},
    {en:"shoot", es:"disparo"},
    {en:"shooting", es:"tiroteo"},
    {en:"shop", es:"tienda"},
    {en:"shopping", es:"compras"},
    {en:"shore", es:"orilla"},
    {en:"short", es:"corto"},
    {en:"shortly", es:"dentro de poco"},
    {en:"shot", es:"Disparo"},
    {en:"should", es:"deberían"},
    {en:"shoulder", es:"hombro"},
    {en:"shout", es:"grito"},
    {en:"show", es:"show"},
    {en:"shower", es:"ducha"},
    {en:"shrug", es:"encogimiento de hombros"},
    {en:"shut", es:"cerrado"},
    {en:"sick", es:"enfermo"},
    {en:"side", es:"lado"},
    {en:"sigh", es:"suspiro"},
    {en:"sight", es:"visión"},
    {en:"sign", es:"firmar"},
    {en:"signal", es:"señal"},
    {en:"significance", es:"significado"},
    {en:"significant", es:"significativo"},
    {en:"significantly", es:"significativamente"},
    {en:"silence", es:"silencio"},
    {en:"silent", es:"silencio"},
    {en:"silver", es:"plata"},
    {en:"similar", es:"similar"},
    {en:"similarly", es:"similar"},
    {en:"simple", es:"simple"},
    {en:"simply", es:"simplemente"},
    {en:"sin", es:"pecado"},
    {en:"since", es:"ya que"},
    {en:"sing", es:"canta"},
    {en:"singer", es:"cantante"},
    {en:"single", es:"único"},
    {en:"sink", es:"lavabo"},
    {en:"sir", es:"señor"},
    {en:"sister", es:"hermana"},
    {en:"sit", es:"sentarse"},
    {en:"site", es:"sitio"},
    {en:"situation", es:"situación"},
    {en:"six", es:"seis"},
    {en:"size", es:"Talla"},
    {en:"ski", es:"esquí"},
    {en:"skill", es:"habilidad"},
    {en:"skin", es:"piel"},
    {en:"sky", es:"cielo"},
    {en:"slave", es:"esclavo"},
    {en:"sleep", es:"dormir"},
    {en:"slice", es:"rodaja"},
    {en:"slide", es:"diapositiva"},
    {en:"slight", es:"leve"},
    {en:"slightly", es:"levemente"},
    {en:"slip", es:"deslizar"},
    {en:"slow", es:"lento"},
    {en:"slowly", es:"despacio"},
    {en:"small", es:"pequeña"},
    {en:"smart", es:"inteligente"},
    {en:"smell", es:"oler"},
    {en:"smile", es:"sonrisa"},
    {en:"smoke", es:"humo"},
    {en:"smooth", es:"suave"},
    {en:"snap", es:"quebrar"},
    {en:"snow", es:"nieve"},
    {en:"so", es:"entonces"},
    {en:"so-called", es:"así llamado"},
    {en:"soccer", es:"fútbol"},
    {en:"social", es:"social"},
    {en:"society", es:"sociedad"},
    {en:"soft", es:"suave"},
    {en:"software", es:"software"},
    {en:"soil", es:"tierra"},
    {en:"solar", es:"solar"},
    {en:"soldier", es:"soldado"},
    {en:"solid", es:"sólido"},
    {en:"solution", es:"solución"},
    {en:"solve", es:"resolver"},
    {en:"some", es:"algunos"},
    {en:"somebody", es:"alguien"},
    {en:"somehow", es:"de algun modo"},
    {en:"someone", es:"alguien"},
    {en:"something", es:"algo"},
    {en:"sometimes", es:"algunas veces"},
    {en:"somewhat", es:"un poco"},
    {en:"somewhere", es:"algun lado"},
    {en:"son", es:"hijo"},
    {en:"song", es:"canción"},
    {en:"soon", es:"pronto"},
    {en:"sophisticated", es:"sofisticado"},
    {en:"sorry", es:"lo siento"},
    {en:"sort", es:"clasificar"},
    {en:"soul", es:"alma"},
    {en:"sound", es:"sonar"},
    {en:"soup", es:"sopa"},
    {en:"source", es:"fuente"},
    {en:"south", es:"Sur"},
    {en:"southern", es:"del Sur"},
    {en:"Soviet", es:"Soviético"},
    {en:"space", es:"espacio"},
    {en:"Spanish", es:"español"},
    {en:"speak", es:"hablar"},
    {en:"speaker", es:"altavoz"},
    {en:"special", es:"especial"},
    {en:"specialist", es:"especialista"},
    {en:"species", es:"especies"},
    {en:"specific", es:"específico"},
    {en:"specifically", es:"específicamente"},
    {en:"speech", es:"habla"},
    {en:"speed", es:"velocidad"},
    {en:"spend", es:"gastar"},
    {en:"spending", es:"gasto"},
    {en:"spin", es:"girar"},
    {en:"spirit", es:"espíritu"},
    {en:"spiritual", es:"espiritual"},
    {en:"split", es:"separar"},
    {en:"spokesman", es:"portavoz"},
    {en:"sport", es:"deporte"},
    {en:"spot", es:"lugar"},
    {en:"spread", es:"untado"},
    {en:"spring", es:"primavera"},
    {en:"square", es:"cuadrado"},
    {en:"squeeze", es:"estrujar"},
    {en:"stability", es:"estabilidad"},
    {en:"stable", es:"estable"},
    {en:"staff", es:"personal"},
    {en:"stage", es:"escenario"},
    {en:"stair", es:"escalera"},
    {en:"stake", es:"apostar"},
    {en:"stand", es:"estar"},
    {en:"standard", es:"estándar"},
    {en:"standing", es:"de pie"},
    {en:"star", es:"estrella"},
    {en:"stare", es:"mirar fijamente"},
    {en:"start", es:"comienzo"},
    {en:"state", es:"Expresar"},
    {en:"statement", es:"declaración"},
    {en:"station", es:"estación"},
    {en:"statistics", es:"Estadísticas"},
    {en:"status", es:"estado"},
    {en:"stay", es:"permanecer"},
    {en:"steady", es:"firme"},
    {en:"steal", es:"robar"},
    {en:"steel", es:"acero"},
    {en:"step", es:"paso"},
    {en:"stick", es:"palo"},
    {en:"still", es:"aún"},
    {en:"stir", es:"revolver"},
    {en:"stock", es:"valores"},
    {en:"stomach", es:"estómago"},
    {en:"stone", es:"Roca"},
    {en:"stop", es:"detener"},
    {en:"storage", es:"almacenamiento"},
    {en:"store", es:"Tienda"},
    {en:"storm", es:"tormenta"},
    {en:"story", es:"historia"},
    {en:"straight", es:"derecho"},
    {en:"strange", es:"extraño"},
    {en:"stranger", es:"extraño"},
    {en:"strategic", es:"estratégico"},
    {en:"strategy", es:"estrategia"},
    {en:"stream", es:"Arroyo"},
    {en:"street", es:"calle"},
    {en:"strength", es:"fuerza"},
    {en:"strengthen", es:"fortalecer"},
    {en:"stress", es:"estrés"},
    {en:"stretch", es:"tramo"},
    {en:"strike", es:"Huelga"},
    {en:"string", es:"cuerda"},
    {en:"strip", es:"banda"},
    {en:"stroke", es:"carrera"},
    {en:"strong", es:"fuerte"},
    {en:"strongly", es:"fuertemente"},
    {en:"structure", es:"estructura"},
    {en:"struggle", es:"lucha"},
    {en:"student", es:"estudiante"},
    {en:"studio", es:"estudio"},
    {en:"study", es:"estudio"},
    {en:"stuff", es:"cosas"},
    {en:"stupid", es:"estúpido"},
    {en:"style", es:"estilo"},
    {en:"subject", es:"sujeto"},
    {en:"submit", es:"entregar"},
    {en:"subsequent", es:"subsecuente"},
    {en:"substance", es:"sustancia"},
    {en:"substantial", es:"sustancial"},
    {en:"succeed", es:"triunfar"},
    {en:"success", es:"éxito"},
    {en:"successful", es:"exitoso"},
    {en:"successfully", es:"exitosamente"},
    {en:"such", es:"semejante"},
    {en:"sudden", es:"repentino"},
    {en:"suddenly", es:"repentinamente"},
    {en:"sue", es:"demandar"},
    {en:"suffer", es:"sufrir"},
    {en:"sufficient", es:"suficiente"},
    {en:"sugar", es:"azúcar"},
    {en:"suggest", es:"sugerir"},
    {en:"suggestion", es:"sugerencia"},
    {en:"suicide", es:"suicidio"},
    {en:"suit", es:"traje"},
    {en:"summer", es:"verano"},
    {en:"summit", es:"cumbre"},
    {en:"sun", es:"sol"},
    {en:"super", es:"súper"},
    {en:"supply", es:"suministro"},
    {en:"support", es:"apoyo"},
    {en:"supporter", es:"seguidor"},
    {en:"suppose", es:"suponer"},
    {en:"supposed", es:"supuesto"},
    {en:"Supreme", es:"Supremo"},
    {en:"sure", es:"seguro"},
    {en:"surely", es:"seguramente"},
    {en:"surface", es:"superficie"},
    {en:"surgery", es:"cirugía"},
    {en:"surprise", es:"sorpresa"},
    {en:"surprised", es:"sorprendido"},
    {en:"surprising", es:"sorprendente"},
    {en:"surprisingly", es:"asombrosamente"},
    {en:"surround", es:"rodear"},
    {en:"survey", es:"encuesta"},
    {en:"survival", es:"supervivencia"},
    {en:"survive", es:"sobrevivir"},
    {en:"survivor", es:"sobreviviente"},
    {en:"suspect", es:"sospechar"},
    {en:"sustain", es:"sostener"},
    {en:"swear", es:"jurar"},
    {en:"sweep", es:"barrer"},
    {en:"sweet", es:"dulce"},
    {en:"swim", es:"nadar"},
    {en:"swing", es:"columpio"},
    {en:"switch", es:"cambiar"},
    {en:"symbol", es:"símbolo"},
    {en:"symptom", es:"síntoma"},
    {en:"system", es:"sistema"},
    {en:"table", es:"mesa"},
    {en:"tablespoon", es:"cucharada"},
    {en:"tactic", es:"táctica"},
    {en:"tail", es:"cola"},
    {en:"take", es:"llevar"},
    {en:"tale", es:"cuento"},
    {en:"talent", es:"talento"},
    {en:"talk", es:"hablar"},
    {en:"tall", es:"alto"},
    {en:"tank", es:"tanque"},
    {en:"tap", es:"grifo"},
    {en:"tape", es:"cinta"},
    {en:"target", es:"objetivo"},
    {en:"task", es:"tarea"},
    {en:"taste", es:"sabor"},
    {en:"tax", es:"impuesto"},
    {en:"taxpayer", es:"contribuyente"},
    {en:"tea", es:"té"},
    {en:"teach", es:"enseñar"},
    {en:"teacher", es:"profesor"},
    {en:"teaching", es:"enseñando"},
    {en:"team", es:"equipo"},
    {en:"tear", es:"lágrima"},
    {en:"teaspoon", es:"cucharilla"},
    {en:"technical", es:"técnico"},
    {en:"technique", es:"técnica"},
    {en:"technology", es:"tecnología"},
    {en:"teen", es:"adolescente"},
    {en:"teenager", es:"adolescente"},
    {en:"telephone", es:"teléfono"},
    {en:"telescope", es:"telescopio"},
    {en:"television", es:"televisión"},
    {en:"tell", es:"contar"},
    {en:"temperature", es:"temperatura"},
    {en:"temporary", es:"temporal"},
    {en:"ten", es:"diez"},
    {en:"tend", es:"tender"},
    {en:"tendency", es:"tendencia"},
    {en:"tennis", es:"tenis"},
    {en:"tension", es:"tensión"},
    {en:"tent", es:"tienda"},
    {en:"term", es:"término"},
    {en:"terms", es:"condiciones"},
    {en:"terrible", es:"terrible"},
    {en:"territory", es:"territorio"},
    {en:"terror", es:"terror"},
    {en:"terrorism", es:"terrorismo"},
    {en:"terrorist", es:"terrorista"},
    {en:"test", es:"prueba"},
    {en:"testify", es:"testificar"},
    {en:"testimony", es:"testimonio"},
    {en:"testing", es:"pruebas"},
    {en:"text", es:"texto"},
    {en:"than", es:"que"},
    {en:"thank", es:"gracias"},
    {en:"thanks", es:"Gracias"},
    {en:"that", es:"ese"},
    {en:"the", es:"el"},
    {en:"theater", es:"teatro"},
    {en:"their", es:"su"},
    {en:"them", es:"ellos"},
    {en:"theme", es:"tema"},
    {en:"themselves", es:"ellos mismos"},
    {en:"then", es:"entonces"},
    {en:"theory", es:"teoría"},
    {en:"therapy", es:"terapia"},
    {en:"there", es:"allí"},
    {en:"therefore", es:"por lo tanto"},
    {en:"these", es:"estos"},
    {en:"they", es:"ellos"},
    {en:"thick", es:"grueso"},
    {en:"thin", es:"Delgado"},
    {en:"thing", es:"cosa"},
    {en:"think", es:"pensar"},
    {en:"thinking", es:"pensando"},
    {en:"third", es:"tercera"},
    {en:"thirty", es:"treinta"},
    {en:"this", es:"esta"},
    {en:"those", es:"aquellos"},
    {en:"though", es:"sin embargo"},
    {en:"thought", es:"pensó"},
    {en:"thousand", es:"mil"},
    {en:"threat", es:"amenaza"},
    {en:"threaten", es:"amenazar"},
    {en:"three", es:"Tres"},
    {en:"throat", es:"garganta"},
    {en:"through", es:"mediante"},
    {en:"throughout", es:"a lo largo de"},
    {en:"throw", es:"lanzar"},
    {en:"thus", es:"por lo tanto"},
    {en:"ticket", es:"boleto"},
    {en:"tie", es:"Corbata"},
    {en:"tight", es:"ajustado"},
    {en:"time", es:"hora"},
    {en:"tiny", es:"diminuto"},
    {en:"tip", es:"propina"},
    {en:"tire", es:"neumático"},
    {en:"tired", es:"cansado"},
    {en:"tissue", es:"pañuelo de papel"},
    {en:"title", es:"título"},
    {en:"to", es:"a"},
    {en:"tobacco", es:"tabaco"},
    {en:"today", es:"hoy dia"},
    {en:"toe", es:"dedo del pie"},
    {en:"together", es:"juntos"},
    {en:"tomato", es:"tomate"},
    {en:"tomorrow", es:"mañana"},
    {en:"tone", es:"tono"},
    {en:"tongue", es:"lengua"},
    {en:"tonight", es:"esta noche"},
    {en:"too", es:"también"},
    {en:"tool", es:"herramienta"},
    {en:"tooth", es:"diente"},
    {en:"top", es:"cima"},
    {en:"topic", es:"tema"},
    {en:"toss", es:"sacudida"},
    {en:"total", es:"total"},
    {en:"totally", es:"totalmente"},
    {en:"touch", es:"tocar"},
    {en:"tough", es:"difícil"},
    {en:"tour", es:"excursión"},
    {en:"tourist", es:"turista"},
    {en:"tournament", es:"torneo"},
    {en:"toward", es:"hacia"},
    {en:"towards", es:"hacia"},
    {en:"tower", es:"torre"},
    {en:"town", es:"pueblo"},
    {en:"toy", es:"juguete"},
    {en:"trace", es:"rastro"},
    {en:"track", es:"pista"},
    {en:"trade", es:"comercio"},
    {en:"tradition", es:"tradicion"},
    {en:"traditional", es:"tradicional"},
    {en:"traffic", es:"tráfico"},
    {en:"tragedy", es:"tragedia"},
    {en:"trail", es:"rastro"},
    {en:"train", es:"tren"},
    {en:"training", es:"capacitación"},
    {en:"transfer", es:"transferir"},
    {en:"transform", es:"transformar"},
    {en:"transformation", es:"transformación"},
    {en:"transition", es:"transición"},
    {en:"translate", es:"traducir"},
    {en:"transportation", es:"transporte"},
    {en:"travel", es:"viaje"},
    {en:"treat", es:"tratar"},
    {en:"treatment", es:"tratamiento"},
    {en:"treaty", es:"tratado"},
    {en:"tree", es:"árbol"},
    {en:"tremendous", es:"tremendo"},
    {en:"trend", es:"tendencia"},
    {en:"trial", es:"juicio"},
    {en:"tribe", es:"tribu"},
    {en:"trick", es:"truco"},
    {en:"trip", es:"viaje"},
    {en:"troop", es:"tropa"},
    {en:"trouble", es:"problema"},
    {en:"truck", es:"camión"},
    {en:"true", es:"cierto"},
    {en:"truly", es:"verdaderamente"},
    {en:"trust", es:"confianza"},
    {en:"truth", es:"verdad"},
    {en:"try", es:"tratar"},
    {en:"tube", es:"tubo"},
    {en:"tunnel", es:"túnel"},
    {en:"turn", es:"turno"},
    {en:"TV", es:"televisión"},
    {en:"twelve", es:"doce"},
    {en:"twenty", es:"veinte"},
    {en:"twice", es:"dos veces"},
    {en:"twin", es:"mellizo"},
    {en:"two", es:"dos"},
    {en:"type", es:"escribe"},
    {en:"typical", es:"típico"},
    {en:"typically", es:"típicamente"},
    {en:"ugly", es:"feo"},
    {en:"ultimate", es:"último"},
    {en:"ultimately", es:"por último"},
    {en:"unable", es:"incapaz"},
    {en:"uncle", es:"tío"},
    {en:"under", es:"bajo"},
    {en:"undergo", es:"someterse"},
    {en:"understand", es:"comprender"},
    {en:"understanding", es:"comprensión"},
    {en:"unfortunately", es:"Desafortunadamente"},
    {en:"uniform", es:"uniforme"},
    {en:"union", es:"Unión"},
    {en:"unique", es:"único"},
    {en:"unit", es:"unidad"},
    {en:"United", es:"Unido"},
    {en:"universal", es:"universal"},
    {en:"universe", es:"universo"},
    {en:"university", es:"Universidad"},
    {en:"unknown", es:"desconocido"},
    {en:"unless", es:"a no ser que"},
    {en:"unlike", es:"a diferencia de"},
    {en:"unlikely", es:"improbable"},
    {en:"until", es:"hasta"},
    {en:"unusual", es:"raro"},
    {en:"up", es:"arriba"},
    {en:"upon", es:"al"},
    {en:"upper", es:"superior"},
    {en:"urban", es:"urbano"},
    {en:"urge", es:"impulso"},
    {en:"us", es:"nosotros"},
    {en:"use", es:"usar"},
    {en:"used", es:"usado"},
    {en:"useful", es:"útil"},
    {en:"user", es:"usuario"},
    {en:"usual", es:"usual"},
    {en:"usually", es:"por lo general"},
    {en:"utility", es:"utilidad"},
    {en:"vacation", es:"vacaciones"},
    {en:"valley", es:"Valle"},
    {en:"valuable", es:"valioso"},
    {en:"value", es:"valor"},
    {en:"variable", es:"variable"},
    {en:"variation", es:"variación"},
    {en:"variety", es:"variedad"},
    {en:"various", es:"varios"},
    {en:"vary", es:"variar"},
    {en:"vast", es:"vasto"},
    {en:"vegetable", es:"vegetal"},
    {en:"vehicle", es:"vehículo"},
    {en:"venture", es:"riesgo"},
    {en:"version", es:"versión"},
    {en:"versus", es:"versus"},
    {en:"very", es:"muy"},
    {en:"vessel", es:"embarcación"},
    {en:"veteran", es:"veterano"},
    {en:"via", es:"vía"},
    {en:"victim", es:"víctima"},
    {en:"victory", es:"victoria"},
    {en:"video", es:"video"},
    {en:"view", es:"vista"},
    {en:"viewer", es:"espectador"},
    {en:"village", es:"pueblo"},
    {en:"violate", es:"violar"},
    {en:"violation", es:"violación"},
    {en:"violence", es:"violencia"},
    {en:"violent", es:"violento"},
    {en:"virtually", es:"virtualmente"},
    {en:"virtue", es:"virtud"},
    {en:"virus", es:"virus"},
    {en:"visible", es:"visible"},
    {en:"vision", es:"visión"},
    {en:"visit", es:"visita"},
    {en:"visitor", es:"visitante"},
    {en:"visual", es:"visual"},
    {en:"vital", es:"vital"},
    {en:"voice", es:"voz"},
    {en:"volume", es:"volumen"},
    {en:"volunteer", es:"voluntario"},
    {en:"vote", es:"votar"},
    {en:"voter", es:"votante"},
    {en:"vs", es:"vs"},
    {en:"vulnerable", es:"vulnerable"},
    {en:"wage", es:"sueldo"},
    {en:"wait", es:"Espere"},
    {en:"wake", es:"despertar"},
    {en:"walk", es:"caminar"},
    {en:"wall", es:"pared"},
    {en:"wander", es:"vagar"},
    {en:"want", es:"desear"},
    {en:"war", es:"guerra"},
    {en:"warm", es:"cálido"},
    {en:"warn", es:"advertir"},
    {en:"warning", es:"advertencia"},
    {en:"wash", es:"lavar"},
    {en:"waste", es:"desperdicio"},
    {en:"watch", es:"reloj"},
    {en:"water", es:"agua"},
    {en:"wave", es:"ola"},
    {en:"way", es:"camino"},
    {en:"we", es:"nosotros"},
    {en:"weak", es:"débil"},
    {en:"wealth", es:"poder"},
    {en:"wealthy", es:"adinerado"},
    {en:"weapon", es:"arma"},
    {en:"wear", es:"llevar"},
    {en:"weather", es:"clima"},
    {en:"wedding", es:"Boda"},
    {en:"week", es:"semana"},
    {en:"weekend", es:"fin de semana"},
    {en:"weekly", es:"semanalmente"},
    {en:"weigh", es:"pesar"},
    {en:"weight", es:"peso"},
    {en:"welcome", es:"bienvenidos"},
    {en:"welfare", es:"bienestar"},
    {en:"well", es:"bien"},
    {en:"west", es:"oeste"},
    {en:"western", es:"occidental"},
    {en:"wet", es:"mojado"},
    {en:"what", es:"qué"},
    {en:"whatever", es:"lo que"},
    {en:"wheel", es:"rueda"},
    {en:"when", es:"Cuándo"},
    {en:"whenever", es:"cuando sea"},
    {en:"where", es:"donde"},
    {en:"whereas", es:"mientras que"},
    {en:"whether", es:"ya sea"},
    {en:"which", es:"cual"},
    {en:"while", es:"mientras"},
    {en:"whisper", es:"susurro"},
    {en:"white", es:"blanco"},
    {en:"who", es:"quién"},
    {en:"whole", es:"entero"},
    {en:"whom", es:"quién"},
    {en:"whose", es:"cuyo"},
    {en:"why", es:"por qué"},
    {en:"wide", es:"ancho"},
    {en:"widely", es:"ampliamente"},
    {en:"widespread", es:"extendido"},
    {en:"wife", es:"esposa"},
    {en:"wild", es:"salvaje"},
    {en:"will", es:"será"},
    {en:"willing", es:"dispuesto"},
    {en:"win", es:"victoria"},
    {en:"wind", es:"viento"},
    {en:"window", es:"ventana"},
    {en:"wine", es:"vino"},
    {en:"wing", es:"ala"},
    {en:"winner", es:"ganador"},
    {en:"winter", es:"invierno"},
    {en:"wipe", es:"pasar un trapo"},
    {en:"wire", es:"cable"},
    {en:"wisdom", es:"sabiduría"},
    {en:"wise", es:"sabio"},
    {en:"wish", es:"deseo"},
    {en:"with", es:"con"},
    {en:"withdraw", es:"retirar"},
    {en:"within", es:"dentro"},
    {en:"without", es:"sin que"},
    {en:"witness", es:"testigo"},
    {en:"woman", es:"mujer"},
    {en:"wonder", es:"preguntarse"},
    {en:"wonderful", es:"maravilloso"},
    {en:"wood", es:"madera"},
    {en:"wooden", es:"de madera"},
    {en:"word", es:"palabra"},
    {en:"work", es:"trabajo"},
    {en:"worker", es:"trabajador"},
    {en:"working", es:"laboral"},
    {en:"works", es:"obras"},
    {en:"workshop", es:"taller"},
    {en:"world", es:"mundo"},
    {en:"worried", es:"preocupado"},
    {en:"worry", es:"preocuparse"},
    {en:"worth", es:"valor"},
    {en:"would", es:"haría"},
    {en:"wound", es:"herida"},
    {en:"wrap", es:"envoltura"},
    {en:"write", es:"escribir"},
    {en:"writer", es:"escritor"},
    {en:"writing", es:"escribiendo"},
    {en:"wrong", es:"incorrecto"},
    {en:"yard", es:"yarda"},
    {en:"yeah", es:"sí"},
    {en:"year", es:"año"},
    {en:"yell", es:"gritar"},
    {en:"yellow", es:"amarillo"},
    {en:"yes", es:"sí"},
    {en:"yesterday", es:"ayer"},
    {en:"yet", es:"aún"},
    {en:"yield", es:"producir"},
    {en:"you", es:"tú"},
    {en:"young", es:"joven"},
    {en:"your", es:"tu"},
    {en:"yours", es:"tuya"},
    {en:"yourself", es:"tú mismo"},
    {en:"youth", es:"juventud"},
    {en:"zone", es:"zona"}
]

export default dictionary;